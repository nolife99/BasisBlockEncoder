...
public static class BlockEncoder
{
    ...
    const uint Bc7AnalyticalMask = 0x3FF;

    static Bcn.Ldr.Bc1Quality MapBc1(Bc1Quality q)
        => q == Bc1Quality.HighQuality ? Bcn.Ldr.Bc1Quality.HighQuality : Bcn.Ldr.Bc1Quality.Fast;

    static Bcn.Hdr.Bc6hQuality MapBc6h(Bc6hQuality q) => q switch
    {
    ...
    static Bcn.Bc7Flags MapBc7(uint flags)
    {
        var m = flags & Bc7AnalyticalMask;
    ...
    /// </summary>
    public static void Encode(BcFormat format,
        ...
        int channel1 = -1)
    {
        var ch0 = channel0 < 0 ? 0 : channel0;
        var ch1 = channel1 < 0 ? 1 : channel1;
        switch (format)
        {
            case BcFormat.Bc1: BlockImage.EncodeBc1(srcRgba, width, height, strideBytes, dst, MapBc1((Bc1Quality)flags)); break;
            case BcFormat.Bc3: BlockImage.EncodeBc3(srcRgba, width, height, strideBytes, dst, MapBc1((Bc1Quality)flags)); break;
            case BcFormat.Bc4: BlockImage.EncodeBc4(srcRgba, width, height, strideBytes, dst, ch0); break;
            case BcFormat.Bc5: BlockImage.EncodeBc5(srcRgba, width, height, strideBytes, dst, ch0, ch1); break;
            case BcFormat.Bc7: BlockImage.EncodeBc7(srcRgba, width, height, strideBytes, dst, MapBc7(flags)); break;
    ...
    public static void EncodeBlock(BcFormat format,
        ReadOnlySpan<byte> block16Rgba,
        Span<byte> dst,
        uint flags = 0,
        int channel0 = -1,
        int channel1 = -1)
    ...
    /// </summary>
    public static void EncodeRows(BcFormat format,
    ...
    public static void EncodeBc1(ReadOnlySpan<byte> rgba,
        int width,
        int height,
        int strideBytes,
        Span<byte> dst,
        Bc1Quality quality = Bc1Quality.HighQuality)
        => BlockImage.EncodeBc1(rgba, width, height, strideBytes, dst, MapBc1(quality));

    /// <summary>Encode RGBA8 to BC3 (BC4 alpha + BC1 color).</summary>
    public static void EncodeBc3(ReadOnlySpan<byte> rgba,
        int width,
        int height,
        int strideBytes,
        Span<byte> dst,
        Bc1Quality quality = Bc1Quality.HighQuality)
        => BlockImage.EncodeBc3(rgba, width, height, strideBytes, dst, MapBc1(quality));

    /// <summary>Encode one channel of RGBA8 to BC4 (default channel 0 = R).</summary>
    public static void EncodeBc4(ReadOnlySpan<byte> rgba,
        int width,
        int height,
        int strideBytes,
        Span<byte> dst,
        int channel = 0)
        => BlockImage.EncodeBc4(rgba, width, height, strideBytes, dst, channel);

    /// <summary>Encode two channels of RGBA8 to BC5 (defaults channel0 = R, channel1 = G).</summary>
    public static void EncodeBc5(ReadOnlySpan<byte> rgba,
        int width,
        int height,
        int strideBytes,
        Span<byte> dst,
        int channel0 = 0,
        int channel1 = 1)
        => BlockImage.EncodeBc5(rgba, width, height, strideBytes, dst, channel0, channel1);

    /// <summary>Encode RGBA8 to BC7 (high quality RGBA).</summary>
    public static void EncodeBc7(ReadOnlySpan<byte> rgba,
        int width,
        int height,
        int strideBytes,
        Span<byte> dst,
        Bc7Flags flags = Bc7Flags.Default)
        => BlockImage.EncodeBc7(rgba, width, height, strideBytes, dst, MapBc7((uint)flags));

    // ---- BC6H (HDR, RGB FP16) ----

    /// <summary>Encode an RGB half-float surface to BC6H. <paramref name="strideBytes" /> == width*3*sizeof(ushort).</summary>
    public static void EncodeBc6h(ReadOnlySpan<Half> rgbHalf,
        int width,
        int height,
        int strideBytes,
        Span<byte> dst,
    ...
    public static void EncodeBc6h(ReadOnlySpan<ushort> rgbHalf,
        int width,
        int height,
        int strideBytes,
        Span<byte> dst,
        Bc6hQuality quality = Bc6hQuality.Default)
        => Bc6hImage.Encode(rgbHalf, width, height, strideBytes / sizeof(ushort), dst, MapBc6h(quality));
    ...
    public static void EncodeBc6hRows(ReadOnlySpan<Half> src,
        int width,
        int pixelRows,
        int strideBytes,
        Span<byte> dst,
    ...
    /// <summary>Encode a band of <paramref name="pixelRows" /> BC6H rows (streaming primitive).</summary>
    public static void EncodeBc6hRows(ReadOnlySpan<ushort> src,
        int width,
        int pixelRows,
        int strideBytes,
        Span<byte> dst,
