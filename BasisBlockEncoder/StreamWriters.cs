...
{
    readonly int _ch0, _ch1, _bpb, _blocksWide;
    readonly Memory<byte> _dst;
    ...
    public BlockRowStreamWriter(BcFormat format,
        int width,
        int height,
        ...
        int channel1 = -1)
    {
        ...
        var need = BlockEncoder.EncodedSize(format, width, height);
    ...
    public bool IsComplete => PixelRowsWritten >= Height;

    /// <summary>Pixel rows not yet written.</summary>
    public int PixelRowsRemaining => Height - PixelRowsWritten;

    /// <summary>Pixel rows written so far.</summary>
    public int PixelRowsWritten { get; private set; }
    ...
    {
        ValidateBand(rows, PixelRowsWritten, Height);
        var bandBlockRows = (rows + 3) / 4;
        var bandBytes = bandBlockRows * _blocksWide * _bpb;
        BlockEncoder.EncodeRows(Format,
            band,
            Width,
            rows,
            strideBytes,
            _dst.Span.Slice(_byteOffset, bandBytes),
            _flags,
            _ch0,
            _ch1);

        _byteOffset += bandBytes;
        PixelRowsWritten += rows;
    }
    ...
    internal static void ValidateBand(int rows, int written, int height)
    {
        ...
        var isFinal = written + rows == height;
        if (!isFinal && (rows & 3) != 0)
...
{
    readonly int _blocksWide;
    readonly Memory<byte> _dst;
    readonly Bc6hQuality _quality;
    int _byteOffset;
    ...
    public Bc6hBlockRowStreamWriter(int width, int height, Memory<byte> destination, Bc6hQuality quality = Bc6hQuality.Default)
    {
        ...
        _quality = quality;
        _blocksWide = BlockEncoder.BlocksWide(width);
        var need = BlockEncoder.EncodedSize(BcFormat.Bc6h, width, height);
        if (destination.Length < need)
            throw new ArgumentException($"Destination too small: need {need} bytes, got {destination.Length}.", nameof(destination));
    }
    ...
    {
        BlockRowStreamWriter.ValidateBand(rows, PixelRowsWritten, Height);
        var bandBlockRows = (rows + 3) / 4;
        var bandBytes = bandBlockRows * _blocksWide * 16;
