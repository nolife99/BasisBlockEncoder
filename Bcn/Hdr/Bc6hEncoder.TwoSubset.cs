...
static partial class Bc6hEncoder
{
    ...
    static readonly uint[] Pats2 = BuildPats2();
    static readonly int[] Anchor2 = BuildAnchor2();

    static readonly int[] ModeOrder2 =
    {
        5, 1
    };

    static readonly int[] ModeOrder4 =
    {
        0, 5, 7, 1
    };

    static readonly int[] ModeOrder9 =
    {
        2, 3, 4, 0, 5, 6, 7, 8, 1
    };

    static uint[] BuildPats2()
    {
        var t = new uint[BC6H_NUM_PATS];
        for (var p = 0; p < BC6H_NUM_PATS; p++)
        {
            uint bits = 0;
            for (var i = 0; i < 16; i++) bits |= (uint)(Bc6hTables.Patterns[p * 16 + i] & 1) << i;
            t[p] = bits;
        }
    ...
    static int[] BuildAnchor2()
    {
        var t = new int[BC6H_NUM_PATS];
        for (var p = 0; p < BC6H_NUM_PATS; p++)
        for (var i = 0; i < 16; i++)
            if (Bc6hTables.Patterns[p * 16 + i] == 0x81)
            {
                t[p] = i;
                break;
            }
        ...
        Params prm)
    {
        ...
        for (var i = 0; i < 16; i++)
        {
            var f = (ir[i] - meanR) * axisR + (ig[i] - meanG) * axisG + (ib[i] - meanB) * axisB;
        ...
        if (prm.Max2SubsetPats == 1)
        {
            ...
            {
                var diff = BitOperations.PopCount(Pats2[p] ^ desired);
                var d = Math.Min(diff, 16 - diff);
                var key = (uint)d << 8 | p;
                if (key < best) best = key;
            }

            var patIndex = (int)(best & 0xFF);
        ...
        else
        {
            ...
            diffs.Sort();
            var n = Math.Min(prm.Max2SubsetPats, BC6H_NUM_PATS);
            for (var it = 0; it < n; it++)
            {
                var patIndex = (int)(diffs[it] & 0xFF);
                TwoSubsetPattern(patIndex, Pats2[patIndex], ir, ig, ib, fr, fg, fb, ps, ref curErr, ref mode, ref partition, ep, w, meanR, meanG, meanB, prm);
            }
        }
    }
        ...
        Params prm)
    {
        ...
        Span<float> minDot = stackalloc float[2]
        {
            BIG_FLOAT_VAL, BIG_FLOAT_VAL
        };

        Span<float> maxDot = stackalloc float[2]
        {
            -BIG_FLOAT_VAL, -BIG_FLOAT_VAL
        };
        ...
        {
            var s = (int)(patBits >> i & 1);
            var dot = ir[i] * axR[s] + ig[i] * axG[s] + ib[i] * axB[s];
            if (dot < minDot[s])
            {
                minDot[s] = dot;
                minIdx[s] = i;
        ...
        if (prm.NumDiffEndpointModes > 0)
        {
            int[] order;
            int numModes;
            if (prm.NumDiffEndpointModes >= 9)
            {
                order = ModeOrder9;
                numModes = 9;
            }
            else if (prm.NumDiffEndpointModes >= 4)
            {
                order = ModeOrder4;
                numModes = 4;
            }
            ...
            for (var mi = 0; mi < numModes; mi++)
            {
                ...
                for (c = 0; c < 3; c++)
                {
                    ...
                    var d0 = e1 - e0;
                    if (d0 < minD || d0 > maxD) break;

                    var d1 = e2 - e0;
                    if (d1 < minD || d1 > maxD) break;

                    var d2 = e3 - e0;
                    if (d2 < minD || d2 > maxD) break;

                    var d3 = e2 - e1;
                    if (d3 < minD || d3 > maxD) break;

                    var d4 = e3 - e1;
                    if (d4 < minD || d4 > maxD) break;
                }

                if (c == 3)
                {
                    bc6hMode = m;
                    numEndpointBits = baseBits;
                    break;
                }
            }
        }
        ...
        var trialErr = AssignWeights3(finalW, patBits, sMin, sMax, fr, fg, fb, ps, true);
        ...
        if ((finalW[0] & 4) != 0)
        {
            for (var c = 0; c < 3; c++) (tep[c * 4 + 0], tep[c * 4 + 1]) = (tep[c * 4 + 1], tep[c * 4 + 0]);
            for (var i = 0; i < 16; i++)
                if ((patBits >> i & 1) == 0)
                    finalW[i] = (byte)(7 - finalW[i]);
        }

        var anchor2 = Anchor2[patIndex];
        if ((finalW[anchor2] & 4) != 0)
        {
            for (var c = 0; c < 3; c++) (tep[c * 4 + 2], tep[c * 4 + 3]) = (tep[c * 4 + 3], tep[c * 4 + 2]);
            for (var i = 0; i < 16; i++)
                if ((patBits >> i & 1) == 1)
                    finalW[i] = (byte)(7 - finalW[i]);
        }
        ...
        if (bc6hMode != ABS_MODE)
        {
            for (var c = 0; c < 3; c++)
            {
                ...
                var mask = (1 << db) - 1;
                var d0 = tep[c * 4 + 1] - tep[c * 4 + 0];
                var d1 = tep[c * 4 + 2] - tep[c * 4 + 0];
                var d2 = tep[c * 4 + 3] - tep[c * 4 + 0];
                tep[c * 4 + 1] = d0 & mask;
                tep[c * 4 + 2] = d1 & mask;
                tep[c * 4 + 3] = d2 & mask;
            }
        ...
        bool computeError)
    {
        ...
        for (var i = 0; i < 16; i++)
        {
            var o = (int)(patBits >> i & 1) * 8;
            float qr = fr[i], qg = fg[i], qb = fb[i];
            var bestE = Sq(cr[o] - qr) + Sq(cg[o] - qg) + Sq(cb[o] - qb);
            var bestIdx = 0;
            for (var j = 1; j < 8; j++)
            {
                var e = Sq(cr[o + j] - qr) + Sq(cg[o + j] - qg) + Sq(cb[o + j] - qb);
                if (e < bestE)
                {
                    bestE = e;
                    bestIdx = j;
