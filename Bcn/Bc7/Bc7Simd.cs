...
static partial class Bc7Block
{
    ...
    internal static readonly bool Bmi2FastPdep = Bmi2.IsSupported && !IsSlowPdepCpu()
        && !(Environment.GetEnvironmentVariable("BC7_DISABLE_PDEP") is "1" or "true" or "TRUE");

    static bool IsSlowPdepCpu()
    {
    ...
    static void ProjMinMax3(scoped ReadOnlySpan<ColorRgba> px, int wr, int wg, int wb, out int loIdx, out int hiIdx)
    {
    ...
    internal static void ProjMinMax3Scalar(scoped ReadOnlySpan<ColorRgba> px, int wr, int wg, int wb, out int loIdx, out int hiIdx)
    {
        ...
        for (var i = 0; i < 16; i++)
        {
            var dot = px[i].R * wr + px[i].G * wg + px[i].B * wb + i;
            if (dot < lo) lo = dot;
            if (dot > hi) hi = dot;
        }
    ...
    {
        var pr = MemoryMarshal.Cast<ColorRgba, uint>(px);
        Vector256<int> wrv = Vector256.Create(wr), wgv = Vector256.Create(wg), wbv = Vector256.Create(wb);
        var mask = Vector256.Create(0xFFu);
        var p0 = Vector256.Create(pr);
        var d0 = (p0 & mask).AsInt32() * wrv + (Vector256.ShiftRightLogical(p0, 8) & mask).AsInt32() * wgv
            + (Vector256.ShiftRightLogical(p0, 16) & mask).AsInt32() * wbv + Vector256.Create(0, 1, 2, 3, 4, 5, 6, 7);

        var p1 = Vector256.Create(pr.Slice(8));
        var d1 = (p1 & mask).AsInt32() * wrv + (Vector256.ShiftRightLogical(p1, 8) & mask).AsInt32() * wgv
            + (Vector256.ShiftRightLogical(p1, 16) & mask).AsInt32() * wbv + Vector256.Create(8, 9, 10, 11, 12, 13, 14, 15);

        loIdx = HMin(Vector256.Min(d0, d1)) & 0xF;
        hiIdx = HMax(Vector256.Max(d0, d1)) & 0xF;
    }
    ...
    internal static void ProjMinMax3V128(scoped ReadOnlySpan<ColorRgba> px, int wr, int wg, int wb, out int loIdx, out int hiIdx)
    {
        ...
        {
            var p = Vector128.Create(pr.Slice(g));
            var d = (p & mask).AsInt32() * wrv + (Vector128.ShiftRightLogical(p, 8) & mask).AsInt32() * wgv
                + (Vector128.ShiftRightLogical(p, 16) & mask).AsInt32() * wbv
                + Vector128.Create(g, g + 1, g + 2, g + 3);
    ...
    static void ProjMinMax4(scoped ReadOnlySpan<ColorRgba> px, int wr, int wg, int wb, int wa, out int loIdx, out int hiIdx)
    {
    ...
    internal static void ProjMinMax4Scalar(scoped ReadOnlySpan<ColorRgba> px, int wr, int wg, int wb, int wa, out int loIdx, out int hiIdx)
    {
        ...
        for (var i = 0; i < 16; i++)
        {
            var dot = px[i].R * wr + px[i].G * wg + px[i].B * wb + px[i].A * wa + i;
            if (dot < lo) lo = dot;
            if (dot > hi) hi = dot;
        }
    ...
    internal static void ProjMinMax4V128(scoped ReadOnlySpan<ColorRgba> px, int wr, int wg, int wb, int wa, out int loIdx, out int hiIdx)
    {
        ...
        {
            var p = Vector128.Create(pr.Slice((int)g));
            var d = (p & mask).AsInt32() * wrv + (Vector128.ShiftRightLogical(p, 8) & mask).AsInt32() * wgv
    ...
    {
        if (AdvSimd.Arm64.IsSupported) return AdvSimd.Arm64.MinAcross(m).ToScalar();
    ...
    {
        if (AdvSimd.Arm64.IsSupported) return AdvSimd.Arm64.MaxAcross(m).ToScalar();
    ...
    internal static void BlockStatsRgbV256(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> s)
    {
        ...
        for (var o = 0; o < 16; o += 8)
        {
            ...
            sG += G;
            sB += B;
            nR = Vector256.Min(nR, R);
            nG = Vector256.Min(nG, G);
            nB = Vector256.Min(nB, B);
            xR = Vector256.Max(xR, R);
            xG = Vector256.Max(xG, G);
            xB = Vector256.Max(xB, B);
        }

        s[0] = Vector256.Sum(sR);
        s[1] = Vector256.Sum(sG);
        s[2] = Vector256.Sum(sB);
        s[3] = HMin(nR);
        s[4] = HMin(nG);
        s[5] = HMin(nB);
        s[6] = HMax(xR);
    ...
    internal static void BlockStatsRgbV128(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> s)
    {
        ...
        {
            var p = Vector128.Create(pru.Slice(o));
            ...
            sG += G;
            sB += B;
            nR = Vector128.Min(nR, R);
            nG = Vector128.Min(nG, G);
            nB = Vector128.Min(nB, B);
            xR = Vector128.Max(xR, R);
            xG = Vector128.Max(xG, G);
            xB = Vector128.Max(xB, B);
        }

        s[0] = Vector128.Sum(sR);
        s[1] = Vector128.Sum(sG);
        s[2] = Vector128.Sum(sB);
        s[3] = HMin(nR);
        s[4] = HMin(nG);
        s[5] = HMin(nB);
        s[6] = HMax(xR);
    ...
    internal static void BlockStatsRgbaV256(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> s)
    {
        ...
        for (var o = 0; o < 16; o += 8)
        {
            ...
            nG = Vector256.Min(nG, G);
            nB = Vector256.Min(nB, B);
            nA = Vector256.Min(nA, A);
            xR = Vector256.Max(xR, R);
            xG = Vector256.Max(xG, G);
            xB = Vector256.Max(xB, B);
            xA = Vector256.Max(xA, A);
        }

        s[0] = Vector256.Sum(sR);
        s[1] = Vector256.Sum(sG);
        s[2] = Vector256.Sum(sB);
        s[3] = Vector256.Sum(sA);
        s[4] = HMin(nR);
        s[5] = HMin(nG);
        s[6] = HMin(nB);
        s[7] = HMin(nA);
    ...
    internal static void BlockStatsRgbaV128(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> s)
    {
        ...
        for (var o = 0; o < 16; o += 4)
        {
            ...
            nG = Vector128.Min(nG, G);
            nB = Vector128.Min(nB, B);
            nA = Vector128.Min(nA, A);
            xR = Vector128.Max(xR, R);
            xG = Vector128.Max(xG, G);
            xB = Vector128.Max(xB, B);
            xA = Vector128.Max(xA, A);
        }

        s[0] = Vector128.Sum(sR);
        s[1] = Vector128.Sum(sG);
        s[2] = Vector128.Sum(sB);
        s[3] = Vector128.Sum(sA);
        s[4] = HMin(nR);
        s[5] = HMin(nG);
        s[6] = HMin(nB);
        s[7] = HMin(nA);
    ...
    static void CovarianceRgb(scoped ReadOnlySpan<ColorRgba> px, int meanR, int meanG, int meanB, scoped Span<int> icov)
    {
    ...
    internal static void CovarianceRgbAvx2(scoped ReadOnlySpan<ColorRgba> px, int meanR, int meanG, int meanB, scoped Span<int> icov)
    {
        ...
        var mask = Vector256.Create(0xFFu);
        Vector256<uint> p0 = Vector256.Create(pr), p1 = Vector256.Create(pr.Slice(8));
        var dr = Avx2.PackSignedSaturate((p0 & mask).AsInt32() - mr, (p1 & mask).AsInt32() - mr);
        var dg = Avx2.PackSignedSaturate((Vector256.ShiftRightLogical(p0, 8) & mask).AsInt32() - mg, (Vector256.ShiftRightLogical(p1, 8) & mask).AsInt32() - mg);
        var db = Avx2.PackSignedSaturate((Vector256.ShiftRightLogical(p0, 16) & mask).AsInt32() - mb, (Vector256.ShiftRightLogical(p1, 16) & mask).AsInt32() - mb);
    ...
    internal static void CovarianceRgbV512(scoped ReadOnlySpan<ColorRgba> px, int meanR, int meanG, int meanB, scoped Span<int> icov)
    {
        ...
        var dg = Avx2.PackSignedSaturate((Vector256.ShiftRightLogical(p0, 8) & mask).AsInt32() - mg, (Vector256.ShiftRightLogical(p1, 8) & mask).AsInt32() - mg);
        var db = Avx2.PackSignedSaturate((Vector256.ShiftRightLogical(p0, 16) & mask).AsInt32() - mb, (Vector256.ShiftRightLogical(p1, 16) & mask).AsInt32() - mb);
        var drdg = Vector512.Create(dr, dg);
        var m1 = Avx512BW.MultiplyAddAdjacent(Vector512.Create(dr, dr), drdg); // rr | rg
        var m2 = Avx512BW.MultiplyAddAdjacent(drdg, Vector512.Create(db, dg)); // rb | gg
        var m3 = Avx512BW.MultiplyAddAdjacent(Vector512.Create(dg, db), Vector512.Create(db, db)); // gb | bb
        icov[0] = Vector256.Sum(m1.GetLower());
        icov[1] = Vector256.Sum(m1.GetUpper());
        icov[2] = Vector256.Sum(m2.GetLower());
        icov[3] = Vector256.Sum(m2.GetUpper());
        icov[4] = Vector256.Sum(m3.GetLower());
        icov[5] = Vector256.Sum(m3.GetUpper());
    }
#endif
    internal static void CovarianceRgbV256(scoped ReadOnlySpan<ColorRgba> px, int meanR, int meanG, int meanB, scoped Span<int> icov)
    {
        ...
        {
            var p = Vector256.Create(pr.Slice((int)g));
            var dr = (p & mask).AsInt32() - mr;
            var dg = (Vector256.ShiftRightLogical(p, 8) & mask).AsInt32() - mg;
            var db = (Vector256.ShiftRightLogical(p, 16) & mask).AsInt32() - mb;
            a0 += dr * dr;
            a1 += dr * dg;
            a2 += dr * db;
            a3 += dg * dg;
        ...
        icov[0] = Vector256.Sum(a0);
        icov[1] = Vector256.Sum(a1);
        icov[2] = Vector256.Sum(a2);
        icov[3] = Vector256.Sum(a3);
        icov[4] = Vector256.Sum(a4);
        icov[5] = Vector256.Sum(a5);
    }
    ...
    internal static void CovarianceRgbV128(scoped ReadOnlySpan<ColorRgba> px, int meanR, int meanG, int meanB, scoped Span<int> icov)
    {
        ...
        for (uint g = 0; g < 16; g += 4)
        {
            var p = Vector128.Create(pr.Slice((int)g));
            var dr = (p & mask).AsInt32() - mr;
            var dg = (Vector128.ShiftRightLogical(p, 8) & mask).AsInt32() - mg;
            var db = (Vector128.ShiftRightLogical(p, 16) & mask).AsInt32() - mb;
            a0 += dr * dr;
            a1 += dr * dg;
            a2 += dr * db;
            a3 += dg * dg;
        ...
        icov[0] = Vector128.Sum(a0);
        icov[1] = Vector128.Sum(a1);
        icov[2] = Vector128.Sum(a2);
        icov[3] = Vector128.Sum(a3);
        icov[4] = Vector128.Sum(a4);
        icov[5] = Vector128.Sum(a5);
    }

    static void CovarianceRgba(scoped ReadOnlySpan<ColorRgba> px, int meanR, int meanG, int meanB, int meanA, scoped Span<int> icov)
    {
#if NET8_0_OR_GREATER
    ...
    internal static void CovarianceRgbaAvx2(scoped ReadOnlySpan<ColorRgba> px, int meanR, int meanG, int meanB, int meanA, scoped Span<int> icov)
    {
        ...
        var dg = Avx2.PackSignedSaturate((Vector256.ShiftRightLogical(p0, 8) & mask).AsInt32() - mg, (Vector256.ShiftRightLogical(p1, 8) & mask).AsInt32() - mg);
        var db = Avx2.PackSignedSaturate((Vector256.ShiftRightLogical(p0, 16) & mask).AsInt32() - mb, (Vector256.ShiftRightLogical(p1, 16) & mask).AsInt32() - mb);
        var da = Avx2.PackSignedSaturate(Vector256.ShiftRightLogical(p0, 24).AsInt32() - ma, Vector256.ShiftRightLogical(p1, 24).AsInt32() - ma);
        icov[0] = Vector256.Sum(Avx2.MultiplyAddAdjacent(dr, dr));
        icov[1] = Vector256.Sum(Avx2.MultiplyAddAdjacent(dr, dg));
        icov[2] = Vector256.Sum(Avx2.MultiplyAddAdjacent(dr, db));
    ...
    internal static void CovarianceRgbaV512(scoped ReadOnlySpan<ColorRgba> px, int meanR, int meanG, int meanB, int meanA, scoped Span<int> icov)
    {
        ...
        var db = Avx2.PackSignedSaturate((Vector256.ShiftRightLogical(p0, 16) & mask).AsInt32() - mb, (Vector256.ShiftRightLogical(p1, 16) & mask).AsInt32() - mb);
        var da = Avx2.PackSignedSaturate(Vector256.ShiftRightLogical(p0, 24).AsInt32() - ma, Vector256.ShiftRightLogical(p1, 24).AsInt32() - ma);
        var m1 = Avx512BW.MultiplyAddAdjacent(Vector512.Create(dr, dr), Vector512.Create(dr, dg)); // rr | rg
        var m2 = Avx512BW.MultiplyAddAdjacent(Vector512.Create(dr, dr), Vector512.Create(db, da)); // rb | ra
        var m3 = Avx512BW.MultiplyAddAdjacent(Vector512.Create(dg, dg), Vector512.Create(dg, db)); // gg | gb
        var m4 = Avx512BW.MultiplyAddAdjacent(Vector512.Create(dg, db), Vector512.Create(da, db)); // ga | bb
        var m5 = Avx512BW.MultiplyAddAdjacent(Vector512.Create(db, da), Vector512.Create(da, da)); // ba | aa
        icov[0] = Vector256.Sum(m1.GetLower());
        icov[1] = Vector256.Sum(m1.GetUpper());
        icov[2] = Vector256.Sum(m2.GetLower());
        icov[3] = Vector256.Sum(m2.GetUpper());
        icov[4] = Vector256.Sum(m3.GetLower());
        icov[5] = Vector256.Sum(m3.GetUpper());
        icov[6] = Vector256.Sum(m4.GetLower());
        icov[7] = Vector256.Sum(m4.GetUpper());
        icov[8] = Vector256.Sum(m5.GetLower());
        icov[9] = Vector256.Sum(m5.GetUpper());
    }
#endif
    internal static void CovarianceRgbaV256(scoped ReadOnlySpan<ColorRgba> px, int meanR, int meanG, int meanB, int meanA, scoped Span<int> icov)
    {
        ...
        for (uint g = 0; g < 16; g += 8)
        {
            ...
            var dg = (Vector256.ShiftRightLogical(p, 8) & mask).AsInt32() - mg;
            var db = (Vector256.ShiftRightLogical(p, 16) & mask).AsInt32() - mb;
            var da = Vector256.ShiftRightLogical(p, 24).AsInt32() - ma;
            a0 += dr * dr;
            a1 += dr * dg;
            a2 += dr * db;
            a3 += dr * da;
        ...
        icov[4] = Vector256.Sum(a4);
        icov[5] = Vector256.Sum(a5);
        icov[6] = Vector256.Sum(a6);
        icov[7] = Vector256.Sum(a7);
        icov[8] = Vector256.Sum(a8);
        icov[9] = Vector256.Sum(a9);
    }
    ...
    internal static void CovarianceRgbaV128(scoped ReadOnlySpan<ColorRgba> px, int meanR, int meanG, int meanB, int meanA, scoped Span<int> icov)
    {
        ...
        for (uint g = 0; g < 16; g += 4)
        {
            ...
            var dg = (Vector128.ShiftRightLogical(p, 8) & mask).AsInt32() - mg;
            var db = (Vector128.ShiftRightLogical(p, 16) & mask).AsInt32() - mb;
            var da = Vector128.ShiftRightLogical(p, 24).AsInt32() - ma;
            a0 += dr * dr;
            a1 += dr * dg;
            a2 += dr * db;
            a3 += dr * da;
        ...
        icov[4] = Vector128.Sum(a4);
        icov[5] = Vector128.Sum(a5);
        icov[6] = Vector128.Sum(a6);
        icov[7] = Vector128.Sum(a7);
        icov[8] = Vector128.Sum(a8);
        icov[9] = Vector128.Sum(a9);
    }
    ...
    {
        ref var p = ref MemoryMarshal.GetReference(px);
        ref var wr = ref MemoryMarshal.GetReference(w);
        for (var i = 0; i < 16; i++)
        {
            ref readonly var q = ref Unsafe.Add(ref p, i);
            var sel = (int)((q.R * dr + q.G * dg + q.B * db + aofs) * f + 0.5f);
    ...
    internal static void EvalSelRgbArm(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, int dr, int dg, int db, int aofs, float f, int mx)
    {
        ...
        for (var o = 0; o < 16; o += 4)
        {
            ...
            Vector128<int> R = (p & mask).AsInt32(), G = (Vector128.ShiftRightLogical(p, 8) & mask).AsInt32(), B = (Vector128.ShiftRightLogical(p, 16) & mask).AsInt32();
            var dot = R * drv + G * dgv + B * dbv + ofsv;
            var selF = AdvSimd.FusedMultiplyAdd(halfv, Vector128.ConvertToSingle(dot), fv);
            var sel = AdvSimd.ConvertToInt32RoundToZero(selF);
    ...
    internal static void EvalSelRgbaScalar(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, int dr, int dg, int db, int da, int aofs, float f, int mx)
    {
        ...
        {
            ref readonly var q = ref Unsafe.Add(ref p, i);
            var sel = (int)((q.R * dr + q.G * dg + q.B * db + q.A * da + aofs) * f + 0.5f);
            if ((uint)sel > (uint)mx) sel = ~sel >> 31 & mx;
            Unsafe.Add(ref wr, i) = sel;
        }
    }
    ...
    internal static void EvalSelRgbaX86(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, int dr, int dg, int db, int da, int aofs, float f, int mx)
    {
        ...
        for (var o = 0; o < 16; o += 8)
        {
            var p = Vector256.Create(pru.Slice(o));
            Vector256<int> R = (p & mask).AsInt32(), G = (Vector256.ShiftRightLogical(p, 8) & mask).AsInt32(), B = (Vector256.ShiftRightLogical(p, 16) & mask).AsInt32(), A = Vector256.ShiftRightLogical(p, 24).AsInt32();
            var dot = R * drv + G * dgv + B * dbv + A * dav + ofsv;
            var selF = Fma.MultiplyAdd(Vector256.ConvertToSingle(dot), fv, halfv);
            var sel = Avx.ConvertToVector256Int32WithTruncation(selF);
            sel = Vector256.Min(Vector256.Max(sel, zero), mxv);
            sel.CopyTo(w.Slice(o));
    ...
    internal static void EvalSelAScalar(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, int la, float f, int mx)
    {
        ...
        for (var i = 0; i < 16; i++)
        {
            var sel = (int)((Unsafe.Add(ref p, i).A - la) * f + 0.5f);
            if ((uint)sel > (uint)mx) sel = ~sel >> 31 & mx;
            Unsafe.Add(ref wr, i) = sel;
        }
    }
    ...
    {
        var pru = MemoryMarshal.Cast<ColorRgba, uint>(px);
        var lav = Vector256.Create(la);
        Vector256<float> fv = Vector256.Create(f), halfv = Vector256.Create(0.5f);
        Vector256<int> zero = Vector256<int>.Zero, mxv = Vector256.Create(mx);
        for (var o = 0; o < 16; o += 8)
        {
            var p = Vector256.Create(pru.Slice(o));
            var dot = Vector256.ShiftRightLogical(p, 24).AsInt32() - lav;
            var selF = Fma.MultiplyAdd(Vector256.ConvertToSingle(dot), fv, halfv);
            var sel = Avx.ConvertToVector256Int32WithTruncation(selF);
            sel = Vector256.Min(Vector256.Max(sel, zero), mxv);
            sel.CopyTo(w.Slice(o));
    ...
    {
        var pru = MemoryMarshal.Cast<ColorRgba, uint>(px);
        var lav = Vector128.Create(la);
        Vector128<float> fv = Vector128.Create(f), halfv = Vector128.Create(0.5f);
        Vector128<int> zero = Vector128<int>.Zero, mxv = Vector128.Create(mx);
        for (var o = 0; o < 16; o += 4)
        {
            var p = Vector128.Create(pru.Slice(o));
            var dot = Vector128.ShiftRightLogical(p, 24).AsInt32() - lav;
            var selF = AdvSimd.FusedMultiplyAdd(halfv, Vector128.ConvertToSingle(dot), fv);
            var sel = AdvSimd.ConvertToInt32RoundToZero(selF);
            sel = Vector128.Min(Vector128.Max(sel, zero), mxv);
            sel.CopyTo(w.Slice(o));
    ...
    internal static void EvalSelRgb2Scalar(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, scoped ReadOnlySpan<int> dr, scoped ReadOnlySpan<int> dg, scoped ReadOnlySpan<int> db, scoped ReadOnlySpan<int> sofs, scoped ReadOnlySpan<float> f, int subsetMask, int mx)
    {
        ...
        for (var i = 0; i < 16; i++)
        {
            var s = subsetMask >> i & 1;
            ref readonly var q = ref Unsafe.Add(ref p, i);
            var sel = (int)((q.R * dr[s] + q.G * dg[s] + q.B * db[s] - sofs[s]) * f[s] + 0.5f);
            if ((uint)sel > (uint)mx) sel = ~sel >> 31 & mx;
            Unsafe.Add(ref wr, i) = sel;
        }
    }
    ...
    internal static void EvalSelRgb2X86(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, scoped ReadOnlySpan<int> dr, scoped ReadOnlySpan<int> dg, scoped ReadOnlySpan<int> db, scoped ReadOnlySpan<int> sofs, scoped ReadOnlySpan<float> f, int subsetMask, int mx)
    {
        ...
        for (var o = 0; o < 16; o += 8)
        {
            var p = Vector256.Create(pru.Slice(o));
            Vector256<int> R = (p & mask).AsInt32(), G = (Vector256.ShiftRightLogical(p, 8) & mask).AsInt32(), B = (Vector256.ShiftRightLogical(p, 16) & mask).AsInt32();
            var s0 = Avx.ConvertToVector256Int32WithTruncation(Fma.MultiplyAdd(Vector256.ConvertToSingle(R * dr0 + G * dg0 + B * db0 - so0), f0, half));
            var s1 = Avx.ConvertToVector256Int32WithTruncation(Fma.MultiplyAdd(Vector256.ConvertToSingle(R * dr1 + G * dg1 + B * db1 - so1), f1, half));
            var m = Vector256.Equals(Vector256.Create(subsetMask >> o & 0xFF) & bitsel, zero);
    ...
    internal static void EvalSelRgb2Arm(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, scoped ReadOnlySpan<int> dr, scoped ReadOnlySpan<int> dg, scoped ReadOnlySpan<int> db, scoped ReadOnlySpan<int> sofs, scoped ReadOnlySpan<float> f, int subsetMask, int mx)
    {
        ...
        for (var o = 0; o < 16; o += 4)
        {
            var p = Vector128.Create(pru.Slice(o));
            Vector128<int> R = (p & mask).AsInt32(), G = (Vector128.ShiftRightLogical(p, 8) & mask).AsInt32(), B = (Vector128.ShiftRightLogical(p, 16) & mask).AsInt32();
            var s0 = AdvSimd.ConvertToInt32RoundToZero(AdvSimd.FusedMultiplyAdd(half, Vector128.ConvertToSingle(R * dr0 + G * dg0 + B * db0 - so0), f0));
            var s1 = AdvSimd.ConvertToInt32RoundToZero(AdvSimd.FusedMultiplyAdd(half, Vector128.ConvertToSingle(R * dr1 + G * dg1 + B * db1 - so1), f1));
            var m = Vector128.Equals(Vector128.Create(subsetMask >> o & 0xF) & bitsel, zero);
            var sel = Vector128.Min(Vector128.Max(Vector128.ConditionalSelect(m, s0, s1), zero), mxv);
            sel.CopyTo(w.Slice(o));
        }
    }
    ...
    internal static void EvalSelRgba2Scalar(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, scoped ReadOnlySpan<int> dr, scoped ReadOnlySpan<int> dg, scoped ReadOnlySpan<int> db, scoped ReadOnlySpan<int> da, scoped ReadOnlySpan<int> sofs, scoped ReadOnlySpan<float> f, int subsetMask, int mx)
    {
        ...
        for (var i = 0; i < 16; i++)
        {
            var s = subsetMask >> i & 1;
            ref readonly var q = ref Unsafe.Add(ref p, i);
            var sel = (int)((q.R * dr[s] + q.G * dg[s] + q.B * db[s] + q.A * da[s] - sofs[s]) * f[s] + 0.5f);
            if ((uint)sel > (uint)mx) sel = ~sel >> 31 & mx;
            Unsafe.Add(ref wr, i) = sel;
        }
    }
    ...
    internal static void EvalSelRgba2X86(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, scoped ReadOnlySpan<int> dr, scoped ReadOnlySpan<int> dg, scoped ReadOnlySpan<int> db, scoped ReadOnlySpan<int> da, scoped ReadOnlySpan<int> sofs, scoped ReadOnlySpan<float> f, int subsetMask, int mx)
    {
        ...
        for (var o = 0; o < 16; o += 8)
        {
            var p = Vector256.Create(pru.Slice(o));
            Vector256<int> R = (p & mask).AsInt32(), G = (Vector256.ShiftRightLogical(p, 8) & mask).AsInt32(), B = (Vector256.ShiftRightLogical(p, 16) & mask).AsInt32(), A = Vector256.ShiftRightLogical(p, 24).AsInt32();
            var s0 = Avx.ConvertToVector256Int32WithTruncation(Fma.MultiplyAdd(Vector256.ConvertToSingle(R * dr0 + G * dg0 + B * db0 + A * da0 - so0), f0, half));
            var s1 = Avx.ConvertToVector256Int32WithTruncation(Fma.MultiplyAdd(Vector256.ConvertToSingle(R * dr1 + G * dg1 + B * db1 + A * da1 - so1), f1, half));
            var m = Vector256.Equals(Vector256.Create(subsetMask >> o & 0xFF) & bitsel, zero);
            var sel = Vector256.Min(Vector256.Max(Vector256.ConditionalSelect(m, s0, s1), zero), mxv);
            sel.CopyTo(w.Slice(o));
        }
    ...
    internal static void EvalSelRgba2Arm(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, scoped ReadOnlySpan<int> dr, scoped ReadOnlySpan<int> dg, scoped ReadOnlySpan<int> db, scoped ReadOnlySpan<int> da, scoped ReadOnlySpan<int> sofs, scoped ReadOnlySpan<float> f, int subsetMask, int mx)
    {
        ...
        for (var o = 0; o < 16; o += 4)
        {
            var p = Vector128.Create(pru.Slice(o));
            Vector128<int> R = (p & mask).AsInt32(), G = (Vector128.ShiftRightLogical(p, 8) & mask).AsInt32(), B = (Vector128.ShiftRightLogical(p, 16) & mask).AsInt32(), A = Vector128.ShiftRightLogical(p, 24).AsInt32();
            var s0 = AdvSimd.ConvertToInt32RoundToZero(AdvSimd.FusedMultiplyAdd(half, Vector128.ConvertToSingle(R * dr0 + G * dg0 + B * db0 + A * da0 - so0), f0));
            var s1 = AdvSimd.ConvertToInt32RoundToZero(AdvSimd.FusedMultiplyAdd(half, Vector128.ConvertToSingle(R * dr1 + G * dg1 + B * db1 + A * da1 - so1), f1));
            var m = Vector128.Equals(Vector128.Create(subsetMask >> o & 0xF) & bitsel, zero);
            var sel = Vector128.Min(Vector128.Max(Vector128.ConditionalSelect(m, s0, s1), zero), mxv);
            sel.CopyTo(w.Slice(o));
        }
    ...
    internal static void EvalSelRgb3X86(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, scoped ReadOnlySpan<int> dr, scoped ReadOnlySpan<int> dg, scoped ReadOnlySpan<int> db, scoped ReadOnlySpan<int> sofs, scoped ReadOnlySpan<float> f, scoped ReadOnlySpan<byte> part, int mx)
    {
        ...
        for (var o = 0; o < 16; o += 8)
        {
            ...
            var s0 = Avx.ConvertToVector256Int32WithTruncation(Fma.MultiplyAdd(Vector256.ConvertToSingle(R * dr0 + G * dg0 + B * db0 - so0), f0, half));
            var s1 = Avx.ConvertToVector256Int32WithTruncation(Fma.MultiplyAdd(Vector256.ConvertToSingle(R * dr1 + G * dg1 + B * db1 - so1), f1, half));
            var s2 = Avx.ConvertToVector256Int32WithTruncation(Fma.MultiplyAdd(Vector256.ConvertToSingle(R * dr2 + G * dg2 + B * db2 - so2), f2, half));
            var idx = Vector256.Create(Unsafe.Add(ref pt, o), Unsafe.Add(ref pt, o + 1), Unsafe.Add(ref pt, o + 2), Unsafe.Add(ref pt, o + 3), Unsafe.Add(ref pt, o + 4), Unsafe.Add(ref pt, o + 5), Unsafe.Add(ref pt, o + 6), Unsafe.Add(ref pt, o + 7));
            var sel = Vector256.ConditionalSelect(Vector256.Equals(idx, two), s2, Vector256.ConditionalSelect(Vector256.Equals(idx, one), s1, s0));
            sel = Vector256.Min(Vector256.Max(sel, zero), mxv);
            sel.CopyTo(w.Slice(o));
        }
    }
    ...
    internal static void EvalSelRgb3Arm(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, scoped ReadOnlySpan<int> dr, scoped ReadOnlySpan<int> dg, scoped ReadOnlySpan<int> db, scoped ReadOnlySpan<int> sofs, scoped ReadOnlySpan<float> f, scoped ReadOnlySpan<byte> part, int mx)
    {
        ...
        for (var o = 0; o < 16; o += 4)
        {
            ...
            var s0 = AdvSimd.ConvertToInt32RoundToZero(AdvSimd.FusedMultiplyAdd(half, Vector128.ConvertToSingle(R * dr0 + G * dg0 + B * db0 - so0), f0));
            var s1 = AdvSimd.ConvertToInt32RoundToZero(AdvSimd.FusedMultiplyAdd(half, Vector128.ConvertToSingle(R * dr1 + G * dg1 + B * db1 - so1), f1));
            var s2 = AdvSimd.ConvertToInt32RoundToZero(AdvSimd.FusedMultiplyAdd(half, Vector128.ConvertToSingle(R * dr2 + G * dg2 + B * db2 - so2), f2));
            var idx = Vector128.Create(Unsafe.Add(ref pt, o), Unsafe.Add(ref pt, o + 1), Unsafe.Add(ref pt, o + 2), Unsafe.Add(ref pt, o + 3));
            var sel = Vector128.ConditionalSelect(Vector128.Equals(idx, two), s2, Vector128.ConditionalSelect(Vector128.Equals(idx, one), s1, s0));
            sel = Vector128.Min(Vector128.Max(sel, zero), mxv);
            sel.CopyTo(w.Slice(o));
        }
    }
    ...
    internal static void LsAccum3DV512Permute(scoped ReadOnlySpan<int> w, scoped ReadOnlySpan<ColorRgba> px, in Bc7Tables.LsCols lc, scoped Span<float> s)
    {
        ...
        var mask = Vector512.Create(0xFFu);
        var p = Vector512.Create(pru);
    ...
    internal static void LsAccum3DAvx2Permute(scoped ReadOnlySpan<int> w, scoped ReadOnlySpan<ColorRgba> px, in Bc7Tables.LsCols lc, scoped Span<float> s)
    {
        var pru = MemoryMarshal.Cast<ColorRgba, uint>(px);
        var mask = Vector256.Create(0xFFu);
        var seven = Vector256.Create(7);
        Vector256<float> z0 = default, z1 = default, z2 = default, qr = default, qg = default, qb = default;
        var wide = lc.Wide;
        for (var o = 0; o < 16; o += 8)
        {
            var wv = Vector256.Create(w.Slice(o));
            Vector256<float> Z0, Z1, Z2, WW;
            if (wide)
            {
                var hi = Avx2.CompareGreaterThan(wv, seven).AsSingle(); // MSB set where w >= 8
            ...
            {
                Z0 = Avx2.PermuteVar8x32(lc.C0, wv);
                Z1 = Avx2.PermuteVar8x32(lc.C1, wv);
                Z2 = Avx2.PermuteVar8x32(lc.C2, wv);
                WW = Avx2.PermuteVar8x32(lc.Cw, wv);
            }
            ...
            z1 += Z1;
            z2 += Z2;
            qr = Fma.MultiplyAdd(WW, Rf, qr);
            qg = Fma.MultiplyAdd(WW, Gf, qg);
            qb = Fma.MultiplyAdd(WW, Bf, qb);
        }

        s[0] = Vector256.Sum(z0);
        s[1] = Vector256.Sum(z1);
        s[2] = Vector256.Sum(z2);
        s[3] = Vector256.Sum(qr);
        s[4] = Vector256.Sum(qg);
        s[5] = Vector256.Sum(qb);
    }
    ...
    {
        ref var wr = ref MemoryMarshal.GetReference(w);
        ref var t = ref MemoryMarshal.GetReference(tab);
        var pru = MemoryMarshal.Cast<ColorRgba, uint>(px);
        var mask = Vector256.Create(0xFFu);
        Vector256<float> z0 = default, z1 = default, z2 = default, qr = default, qg = default, qb = default;
        for (var o = 0; o < 16; o += 8)
        {
            ...
            var Z0 = Vector256.Create(Unsafe.Add(ref t, o0), Unsafe.Add(ref t, o1), Unsafe.Add(ref t, o2), Unsafe.Add(ref t, o3), Unsafe.Add(ref t, o4), Unsafe.Add(ref t, o5), Unsafe.Add(ref t, o6), Unsafe.Add(ref t, o7));
            var Z1 = Vector256.Create(Unsafe.Add(ref t, o0 + 1), Unsafe.Add(ref t, o1 + 1), Unsafe.Add(ref t, o2 + 1), Unsafe.Add(ref t, o3 + 1), Unsafe.Add(ref t, o4 + 1), Unsafe.Add(ref t, o5 + 1), Unsafe.Add(ref t, o6 + 1), Unsafe.Add(ref t, o7 + 1));
            var Z2 = Vector256.Create(Unsafe.Add(ref t, o0 + 2), Unsafe.Add(ref t, o1 + 2), Unsafe.Add(ref t, o2 + 2), Unsafe.Add(ref t, o3 + 2), Unsafe.Add(ref t, o4 + 2), Unsafe.Add(ref t, o5 + 2), Unsafe.Add(ref t, o6 + 2), Unsafe.Add(ref t, o7 + 2));
            var WW = Vector256.Create(Unsafe.Add(ref t, o0 + 3), Unsafe.Add(ref t, o1 + 3), Unsafe.Add(ref t, o2 + 3), Unsafe.Add(ref t, o3 + 3), Unsafe.Add(ref t, o4 + 3), Unsafe.Add(ref t, o5 + 3), Unsafe.Add(ref t, o6 + 3), Unsafe.Add(ref t, o7 + 3));
            var p = Vector256.Create(pru.Slice(o));
            Vector256<float> Rf = Vector256.ConvertToSingle((p & mask).AsInt32()), Gf = Vector256.ConvertToSingle((Vector256.ShiftRightLogical(p, 8) & mask).AsInt32()), Bf = Vector256.ConvertToSingle((Vector256.ShiftRightLogical(p, 16) & mask).AsInt32());
            z0 += Z0;
            z1 += Z1;
    ...
    internal static void LsAccum3DArm(scoped ReadOnlySpan<int> w, scoped ReadOnlySpan<ColorRgba> px, scoped ReadOnlySpan<float> tab, scoped Span<float> s)
    {
        ...
        for (var o = 0; o < 16; o += 4)
        {
            ...
            var Z0 = Vector128.Create(Unsafe.Add(ref t, o0), Unsafe.Add(ref t, o1), Unsafe.Add(ref t, o2), Unsafe.Add(ref t, o3));
            var Z1 = Vector128.Create(Unsafe.Add(ref t, o0 + 1), Unsafe.Add(ref t, o1 + 1), Unsafe.Add(ref t, o2 + 1), Unsafe.Add(ref t, o3 + 1));
            var Z2 = Vector128.Create(Unsafe.Add(ref t, o0 + 2), Unsafe.Add(ref t, o1 + 2), Unsafe.Add(ref t, o2 + 2), Unsafe.Add(ref t, o3 + 2));
            var WW = Vector128.Create(Unsafe.Add(ref t, o0 + 3), Unsafe.Add(ref t, o1 + 3), Unsafe.Add(ref t, o2 + 3), Unsafe.Add(ref t, o3 + 3));
            var p = Vector128.Create(pru.Slice(o));
            ...
            z1 += Z1;
            z2 += Z2;
            qr = AdvSimd.FusedMultiplyAdd(qr, WW, Rf);
            qg = AdvSimd.FusedMultiplyAdd(qg, WW, Gf);
            qb = AdvSimd.FusedMultiplyAdd(qb, WW, Bf);
        }

        s[0] = Vector128.Sum(z0);
        s[1] = Vector128.Sum(z1);
        s[2] = Vector128.Sum(z2);
        s[3] = Vector128.Sum(qr);
        s[4] = Vector128.Sum(qg);
        s[5] = Vector128.Sum(qb);
    }
    ...
    internal static void LsAccum4DAvx2Permute(scoped ReadOnlySpan<int> w, scoped ReadOnlySpan<ColorRgba> px, in Bc7Tables.LsCols lc, scoped Span<float> s)
    {
        ...
        for (var o = 0; o < 16; o += 8)
        {
            ...
            if (wide)
            {
                var hi = Avx2.CompareGreaterThan(wv, seven).AsSingle();
                Z0 = Avx2.BlendVariable(Avx2.PermuteVar8x32(lc.C0, wv), Avx2.PermuteVar8x32(lc.C0h, wv), hi);
                Z1 = Avx2.BlendVariable(Avx2.PermuteVar8x32(lc.C1, wv), Avx2.PermuteVar8x32(lc.C1h, wv), hi);
                Z2 = Avx2.BlendVariable(Avx2.PermuteVar8x32(lc.C2, wv), Avx2.PermuteVar8x32(lc.C2h, wv), hi);
                WW = Avx2.BlendVariable(Avx2.PermuteVar8x32(lc.Cw, wv), Avx2.PermuteVar8x32(lc.Cwh, wv), hi);
    ...
    internal static void LsAccum4DArm(scoped ReadOnlySpan<int> w, scoped ReadOnlySpan<ColorRgba> px, scoped ReadOnlySpan<float> tab, scoped Span<float> s)
    {
        ...
        for (var o = 0; o < 16; o += 4)
        {
            ...
            qg = AdvSimd.FusedMultiplyAdd(qg, WW, Gf);
            qb = AdvSimd.FusedMultiplyAdd(qb, WW, Bf);
            qa = AdvSimd.FusedMultiplyAdd(qa, WW, Af);
        }
    ...
    {
        var ra = MemoryMarshal.AsBytes(a);
        var rb = MemoryMarshal.AsBytes(b);
        var acc = Vector256<int>.Zero;
        for (var o = 0; o < 64; o += 32)
        {
            Vector256<byte> av = Vector256.Create(ra.Slice(o)), bv = Vector256.Create(rb.Slice(o));
            var dLo = Avx2.ConvertToVector256Int16(av.GetLower()) - Avx2.ConvertToVector256Int16(bv.GetLower());
            var dHi = Avx2.ConvertToVector256Int16(av.GetUpper()) - Avx2.ConvertToVector256Int16(bv.GetUpper());
    ...
    internal static long SsdRgbaVnni(scoped ReadOnlySpan<ColorRgba> a, scoped ReadOnlySpan<ColorRgba> b)
    {
        ...
        {
            Vector256<byte> av = Vector256.Create(ra.Slice(o)), bv = Vector256.Create(rb.Slice(o));
            var dLo = Vector256.WidenLower(av).AsInt16() - Vector256.WidenLower(bv).AsInt16();
            var dHi = Vector256.WidenUpper(av).AsInt16() - Vector256.WidenUpper(bv).AsInt16();
    ...
    {
        var ar = MemoryMarshal.Cast<ColorRgba, uint>(a);
        var br = MemoryMarshal.Cast<ColorRgba, uint>(b);
        var m = Vector256.Create(0xFFu);
        var acc = Vector256<int>.Zero;
        for (var o = 0; o < 16; o += 8)
        {
            Vector256<uint> p = Vector256.Create(ar.Slice(o)), q = Vector256.Create(br.Slice(o));
            var dR = (p & m).AsInt32() - (q & m).AsInt32();
            var dG = (Vector256.ShiftRightLogical(p, 8) & m).AsInt32() - (Vector256.ShiftRightLogical(q, 8) & m).AsInt32();
            var dB = (Vector256.ShiftRightLogical(p, 16) & m).AsInt32() - (Vector256.ShiftRightLogical(q, 16) & m).AsInt32();
            var dA = Vector256.ShiftRightLogical(p, 24).AsInt32() - Vector256.ShiftRightLogical(q, 24).AsInt32();
    ...
    internal static long SsdRgbaV128(scoped ReadOnlySpan<ColorRgba> a, scoped ReadOnlySpan<ColorRgba> b)
    {
        var ar = MemoryMarshal.Cast<ColorRgba, uint>(a);
        var br = MemoryMarshal.Cast<ColorRgba, uint>(b);
        var m = Vector128.Create(0xFFu);
        var acc = Vector128<int>.Zero;
        for (var o = 0; o < 16; o += 4)
        {
            Vector128<uint> p = Vector128.Create(ar.Slice(o)), q = Vector128.Create(br.Slice(o));
            var dR = (p & m).AsInt32() - (q & m).AsInt32();
            var dG = (Vector128.ShiftRightLogical(p, 8) & m).AsInt32() - (Vector128.ShiftRightLogical(q, 8) & m).AsInt32();
            var dB = (Vector128.ShiftRightLogical(p, 16) & m).AsInt32() - (Vector128.ShiftRightLogical(q, 16) & m).AsInt32();
            var dA = Vector128.ShiftRightLogical(p, 24).AsInt32() - Vector128.ShiftRightLogical(q, 24).AsInt32();
            acc += dR * dR + dG * dG + dB * dB + dA * dA;
        }
    ...
    internal static long SsdRgbaArm(scoped ReadOnlySpan<ColorRgba> a, scoped ReadOnlySpan<ColorRgba> b)
    {
        ...
        for (var o = 0; o < 64; o += 16)
        {
            Vector128<byte> av = Vector128.Create(ra.Slice(o)), bv = Vector128.Create(rb.Slice(o));
            var d = AdvSimd.AbsoluteDifference(av, bv);
    ...
    internal static void EvalSelRgbV512(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, int dr, int dg, int db, int aofs, float f, int mx)
    {
        ...
        var dot = R * drv + G * dgv + B * dbv + ofsv;
        var sel = Avx512F.ConvertToVector512Int32WithTruncation(Avx512F.FusedMultiplyAdd(Vector512.ConvertToSingle(dot), fv, halfv));
    ...
    {
        var pru = MemoryMarshal.Cast<ColorRgba, uint>(px);
        var lav = Vector512.Create(la);
        Vector512<float> fv = Vector512.Create(f), halfv = Vector512.Create(0.5f);
        var p = Vector512.Create(pru);
        var dot = Vector512.ShiftRightLogical(p, 24).AsInt32() - lav;
        var sel = Avx512F.ConvertToVector512Int32WithTruncation(Avx512F.FusedMultiplyAdd(Vector512.ConvertToSingle(dot), fv, halfv));
        sel = Vector512.Min(Vector512.Max(sel, Vector512<int>.Zero), Vector512.Create(mx));
        sel.CopyTo(w);
    }
    ...
    internal static void EvalSelRgb2V512(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, scoped ReadOnlySpan<int> dr, scoped ReadOnlySpan<int> dg, scoped ReadOnlySpan<int> db, scoped ReadOnlySpan<int> sofs, scoped ReadOnlySpan<float> f, int subsetMask, int mx)
    {
        ...
        var bitsel = Vector512.Create(1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768);
        var p = Vector512.Create(pru);
        Vector512<int> R = (p & mask).AsInt32(), G = (Vector512.ShiftRightLogical(p, 8) & mask).AsInt32(), B = (Vector512.ShiftRightLogical(p, 16) & mask).AsInt32();
        var s0 = Avx512F.ConvertToVector512Int32WithTruncation(Avx512F.FusedMultiplyAdd(Vector512.ConvertToSingle(R * dr0 + G * dg0 + B * db0 - so0), f0, half));
        var s1 = Avx512F.ConvertToVector512Int32WithTruncation(Avx512F.FusedMultiplyAdd(Vector512.ConvertToSingle(R * dr1 + G * dg1 + B * db1 - so1), f1, half));
        var m = Vector512.Equals(Vector512.Create(subsetMask & 0xFFFF) & bitsel, Vector512<int>.Zero);
        var sel = Vector512.Min(Vector512.Max(Vector512.ConditionalSelect(m, s0, s1), Vector512<int>.Zero), Vector512.Create(mx));
        sel.CopyTo(w);
    }
    ...
    internal static void EvalSelRgba2V512(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, scoped ReadOnlySpan<int> dr, scoped ReadOnlySpan<int> dg, scoped ReadOnlySpan<int> db, scoped ReadOnlySpan<int> da, scoped ReadOnlySpan<int> sofs, scoped ReadOnlySpan<float> f, int subsetMask, int mx)
    {
        ...
        var p = Vector512.Create(pru);
        Vector512<int> R = (p & mask).AsInt32(), G = (Vector512.ShiftRightLogical(p, 8) & mask).AsInt32(), B = (Vector512.ShiftRightLogical(p, 16) & mask).AsInt32(), A = Vector512.ShiftRightLogical(p, 24).AsInt32();
        var s0 = Avx512F.ConvertToVector512Int32WithTruncation(Avx512F.FusedMultiplyAdd(Vector512.ConvertToSingle(R * dr0 + G * dg0 + B * db0 + A * da0 - so0), f0, half));
        var s1 = Avx512F.ConvertToVector512Int32WithTruncation(Avx512F.FusedMultiplyAdd(Vector512.ConvertToSingle(R * dr1 + G * dg1 + B * db1 + A * da1 - so1), f1, half));
        var m = Vector512.Equals(Vector512.Create(subsetMask & 0xFFFF) & bitsel, Vector512<int>.Zero);
        var sel = Vector512.Min(Vector512.Max(Vector512.ConditionalSelect(m, s0, s1), Vector512<int>.Zero), Vector512.Create(mx));
        sel.CopyTo(w);
    }
    ...
    internal static void EvalSelRgb3V512(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, scoped ReadOnlySpan<int> dr, scoped ReadOnlySpan<int> dg, scoped ReadOnlySpan<int> db, scoped ReadOnlySpan<int> sofs, scoped ReadOnlySpan<float> f, scoped ReadOnlySpan<byte> part, int mx)
    {
        ...
        var s0 = Avx512F.ConvertToVector512Int32WithTruncation(Avx512F.FusedMultiplyAdd(Vector512.ConvertToSingle(R * dr0 + G * dg0 + B * db0 - so0), f0, half));
        var s1 = Avx512F.ConvertToVector512Int32WithTruncation(Avx512F.FusedMultiplyAdd(Vector512.ConvertToSingle(R * dr1 + G * dg1 + B * db1 - so1), f1, half));
        var s2 = Avx512F.ConvertToVector512Int32WithTruncation(Avx512F.FusedMultiplyAdd(Vector512.ConvertToSingle(R * dr2 + G * dg2 + B * db2 - so2), f2, half));
        var idx = Vector512.Create(Unsafe.Add(ref pt, 0),
            Unsafe.Add(ref pt, 1),
            Unsafe.Add(ref pt, 2),
            Unsafe.Add(ref pt, 3),
            Unsafe.Add(ref pt, 4),
            ...
            Unsafe.Add(ref pt, 8),
            Unsafe.Add(ref pt, 9),
            Unsafe.Add(ref pt, 10),
            Unsafe.Add(ref pt, 11),
            Unsafe.Add(ref pt, 12),
            Unsafe.Add(ref pt, 13),
            Unsafe.Add(ref pt, 14),
            Unsafe.Add(ref pt, 15));

        var sel = Vector512.ConditionalSelect(Vector512.Equals(idx, two), s2, Vector512.ConditionalSelect(Vector512.Equals(idx, one), s1, s0));
        sel = Vector512.Min(Vector512.Max(sel, Vector512<int>.Zero), Vector512.Create(mx));
        sel.CopyTo(w);
    }
#endif
    ...
    internal static long SsdRgbaV512(scoped ReadOnlySpan<ColorRgba> a, scoped ReadOnlySpan<ColorRgba> b)
    {
        var ar = MemoryMarshal.Cast<ColorRgba, uint>(a);
        var br = MemoryMarshal.Cast<ColorRgba, uint>(b);
        var m = Vector512.Create(0xFFu);
        Vector512<uint> p = Vector512.Create(ar), q = Vector512.Create(br);
        var dR = (p & m).AsInt32() - (q & m).AsInt32();
        var dG = (Vector512.ShiftRightLogical(p, 8) & m).AsInt32() - (Vector512.ShiftRightLogical(q, 8) & m).AsInt32();
        var dB = (Vector512.ShiftRightLogical(p, 16) & m).AsInt32() - (Vector512.ShiftRightLogical(q, 16) & m).AsInt32();
        var dA = Vector512.ShiftRightLogical(p, 24).AsInt32() - Vector512.ShiftRightLogical(q, 24).AsInt32();
    ...
    internal static long SsdRgbaV512Bw(scoped ReadOnlySpan<ColorRgba> a, scoped ReadOnlySpan<ColorRgba> b)
    {
        var ra = MemoryMarshal.AsBytes(a);
        var rb = MemoryMarshal.AsBytes(b);
        Vector512<byte> av = Vector512.Create(ra), bv = Vector512.Create(rb);
        var dLo = Vector512.WidenLower(av).AsInt16() - Vector512.WidenLower(bv).AsInt16();
        var dHi = Vector512.WidenUpper(av).AsInt16() - Vector512.WidenUpper(bv).AsInt16();
