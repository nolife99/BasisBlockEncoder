...
static class Bc7Decoder
{
    ...
    static int From7(int v, int p) => v << 1 | p;

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static int From6(int v, int p)
    {
        var t = (uint)v << 1 | (uint)p;
        t = t << 1 | t >> 6;
        return (int)t;
    }
    ...
    static int Bits(ulong lo, ulong hi, int start, int len)
    {
        ...
        {
            v = lo >> start;
            if (start + len > 64) v |= hi << 64 - start;
        }
        else v = hi >> start - 64;

        var mask = len == 64 ? ~0UL : (1UL << len) - 1;
        return (int)(v & mask);
    }
    ...
    {
        ulong lo = 0, hi = 0;
        for (var i = 0; i < 8; i++) lo |= (ulong)blk[i] << 8 * i;
        for (var i = 0; i < 8; i++) hi |= (ulong)blk[8 + i] << 8 * i;
        return (lo, hi);
    }
    ...
    static void DecodeMode6(scoped ReadOnlySpan<byte> blk, scoped Span<ColorRgba> o)
    {
        ...
        var bit = 65;
        ref var op = ref MemoryMarshal.GetReference(o);
        ref var W = ref MemoryMarshal.GetArrayDataReference(Bc7Tables.Weights4);
        for (var i = 0; i < 16; i++)
        {
            var len = i == 0 ? 3 : 4;
            var w = Unsafe.Add(ref W, Bits(lo, hi, bit, len));
            bit += len;
            Unsafe.Add(ref op, i) = new((byte)Lerp(lr8, hr8, w), (byte)Lerp(lg8, hg8, w), (byte)Lerp(lb8, hb8, w), (byte)Lerp(la8, ha8, w));
        }
    }
    ...
    static void DecodeMode3(scoped ReadOnlySpan<byte> blk, scoped Span<ColorRgba> o)
    {
        ...
        lr[0] = From7(lr0, p0);
        lr[1] = From7(lr1, p2);
        Span<int> hr = stackalloc int[2];
        hr[0] = From7(hr0, p1);
        hr[1] = From7(hr1, p3);
        Span<int> lg = stackalloc int[2];
        lg[0] = From7(lg0, p0);
        lg[1] = From7(lg1, p2);
        Span<int> hg = stackalloc int[2];
        hg[0] = From7(hg0, p1);
        hg[1] = From7(hg1, p3);
        Span<int> lb = stackalloc int[2];
        lb[0] = From7(lb0, p0);
        lb[1] = From7(lb1, p2);
        Span<int> hb = stackalloc int[2];
        hb[0] = From7(hb0, p1);
        hb[1] = From7(hb1, p3);
        int anchor = Bc7Tables.AnchorIndexSecondSubset[part];
        var bit = 98;
        ref var op = ref MemoryMarshal.GetReference(o);
        ref var P = ref MemoryMarshal.GetArrayDataReference(Bc7Tables.Partition2);
        ref var W = ref MemoryMarshal.GetArrayDataReference(Bc7Tables.Weights2);
        for (var i = 0; i < 16; i++)
        {
            int s = Unsafe.Add(ref P, part * 16 + i);
            var len = i == 0 || i == anchor ? 1 : 2;
            var w = Unsafe.Add(ref W, Bits(lo, hi, bit, len));
            bit += len;
            Unsafe.Add(ref op, i) = new((byte)Lerp(lr[s], hr[s], w), (byte)Lerp(lg[s], hg[s], w), (byte)Lerp(lb[s], hb[s], w), 255);
        }
    }
    ...
    static void DecodeMode1(scoped ReadOnlySpan<byte> blk, scoped Span<ColorRgba> o)
    {
        ...
        ref var op = ref MemoryMarshal.GetReference(o);
        ref var P = ref MemoryMarshal.GetArrayDataReference(Bc7Tables.Partition2);
        ref var W = ref MemoryMarshal.GetArrayDataReference(Bc7Tables.Weights3);
        for (var i = 0; i < 16; i++)
        {
            int s = Unsafe.Add(ref P, part * 16 + i);
            var len = i == 0 || i == anchor ? 2 : 3;
            var w = Unsafe.Add(ref W, Bits(lo, hi, bit, len));
            bit += len;
            Unsafe.Add(ref op, i) = new((byte)Lerp(lr[s], hr[s], w), (byte)Lerp(lg[s], hg[s], w), (byte)Lerp(lb[s], hb[s], w), 255);
        }
    ...
    static void DecodeMode0(scoped ReadOnlySpan<byte> blk, scoped Span<ColorRgba> o)
    {
        ...
        lb[1] = From4(lb1, p2);
        lb[2] = From4(lb2, p4);
        Span<int> hb = stackalloc int[3];
        hb[0] = From4(hb0, p1);
        hb[1] = From4(hb1, p3);
        hb[2] = From4(hb2, p5);
        int a0 = Bc7Tables.AnchorThird1[part], a1 = Bc7Tables.AnchorThird2[part];
        var bit = 83;
        ref var op = ref MemoryMarshal.GetReference(o);
        ref var P = ref MemoryMarshal.GetArrayDataReference(Bc7Tables.Partition3);
        ref var W = ref MemoryMarshal.GetArrayDataReference(Bc7Tables.Weights3);
        for (var i = 0; i < 16; i++)
        {
            int s = Unsafe.Add(ref P, part * 16 + i);
            var len = i == 0 || i == a0 || i == a1 ? 2 : 3;
            var w = Unsafe.Add(ref W, Bits(lo, hi, bit, len));
            bit += len;
            Unsafe.Add(ref op, i) = new((byte)Lerp(lr[s], hr[s], w), (byte)Lerp(lg[s], hg[s], w), (byte)Lerp(lb[s], hb[s], w), 255);
        }
    ...
    static void DecodeMode5(scoped ReadOnlySpan<byte> blk, scoped Span<ColorRgba> o)
    {
        ...
        {
            var cl = i == 0 ? 1 : 2;
            var wc = Unsafe.Add(ref W, Bits(lo, hi, cb, cl));
            cb += cl;
            var al = i == 0 ? 1 : 2;
            var wa = Unsafe.Add(ref W, Bits(lo, hi, ab, al));
            ab += al;
            Unsafe.Add(ref op, i) = new((byte)Lerp(r0, r1, wc), (byte)Lerp(g0, g1, wc), (byte)Lerp(b0, b1, wc), (byte)Lerp(la, ha, wa));
        }
    ...
    {
        var (lo, hi) = Load(blk);
        var rot = Bits(lo, hi, 5, 2);
        var flag = Bits(lo, hi, 7, 1);
        ...
        Span<int> idx3 = stackalloc int[16];
        int b2 = 50, b3 = 81;
        ref var I2 = ref MemoryMarshal.GetReference(idx2);
        ref var I3 = ref MemoryMarshal.GetReference(idx3);
        for (var i = 0; i < 16; i++)
        {
            var l = i == 0 ? 1 : 2;
            Unsafe.Add(ref I2, i) = Bits(lo, hi, b2, l);
            b2 += l;
        }
        ...
        {
            var l = i == 0 ? 2 : 3;
            Unsafe.Add(ref I3, i) = Bits(lo, hi, b3, l);
            b3 += l;
        }

        ref var op = ref MemoryMarshal.GetReference(o);
        ref var W2 = ref MemoryMarshal.GetArrayDataReference(Bc7Tables.Weights2);
        ref var W3 = ref MemoryMarshal.GetArrayDataReference(Bc7Tables.Weights3);
        for (var i = 0; i < 16; i++)
        {
            var wc = flag != 0 ? Unsafe.Add(ref W3, Unsafe.Add(ref I3, i)) : Unsafe.Add(ref W2, Unsafe.Add(ref I2, i));
            var wa = flag != 0 ? Unsafe.Add(ref W2, Unsafe.Add(ref I2, i)) : Unsafe.Add(ref W3, Unsafe.Add(ref I3, i));
            Unsafe.Add(ref op, i) = new((byte)Lerp(r0, r1, wc), (byte)Lerp(g0, g1, wc), (byte)Lerp(b0, b1, wc), (byte)Lerp(a0, a1, wa));
        }
    ...
    static void DecodeMode7(scoped ReadOnlySpan<byte> blk, scoped Span<ColorRgba> o)
    {
        ...
        for (var i = 0; i < 16; i++)
        {
            ...
            var w = Unsafe.Add(ref W, Bits(lo, hi, bit, len));
            bit += len;
            Unsafe.Add(ref op, i) = new((byte)Lerp(lr[s], hr[s], w), (byte)Lerp(lg[s], hg[s], w), (byte)Lerp(lb[s], hb[s], w), (byte)Lerp(la[s], ha[s], w));
        }
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static int ModeOf(byte b0) => (b0 & 1) != 0 ? 0 :
        (b0 & 2) != 0 ? 1 :
        (b0 & 4) != 0 ? 2 :
        (b0 & 8) != 0 ? 3 :
        (b0 & 0x10) != 0 ? 4 :
    ...
    public static uint ActualSse(scoped ReadOnlySpan<byte> block, scoped ReadOnlySpan<ColorRgba> px)
    {
        ...
        var sse = Bc7Block.SsdRgba(px, dec);
