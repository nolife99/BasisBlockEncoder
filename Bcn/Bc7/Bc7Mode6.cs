...
internal static partial class Bc7Block
{
    ...
    private static void PackMode6Default(scoped ReadOnlySpan<ColorRgba> px, scoped Span<byte> block,
        float axR, float axG, float axB, int totR, int totG, int totB, Bc7Flags flags,
        bool wantSse, out uint trueSse)
    {
        ...
        float k = MathF.Max(MathF.Abs(axR), MathF.Max(MathF.Abs(axG), MathF.Abs(axB)));
        if (k >= SmallFloat) { float m = 2048f / k; sr = (int)(axR * m); sg = (int)(axG * m); sb = (int)(axB * m); }
        sr <<= 4; sg <<= 4; sb <<= 4;

        ProjMinMax3(px, sr, sg, sb, out int loIdx, out int hiIdx);
        ...
        uint p0, p1; int lr, lg, lb, hr, hg, hb;
        if (pbitOptM6)
        {
            sxl[0]=px[loIdx].R/255f; sxl[1]=px[loIdx].G/255f; sxl[2]=px[loIdx].B/255f; sxl[3]=0f;
            sxh[0]=px[hiIdx].R/255f; sxh[1]=px[hiIdx].G/255f; sxh[2]=px[hiIdx].B/255f; sxh[3]=0f;
            DetermineUniquePbits(3, 7, sxl, sxh, minC, maxC, pbo);
            p0=pbo[0]; p1=pbo[1]; lr=minC[0]; lg=minC[1]; lb=minC[2]; hr=maxC[0]; hg=maxC[1]; hb=maxC[2];
        }
        else
        {
            p0 = 1; p1 = 1; // fixed p-bits (keeps alpha 255; the p-bit search isn't worth its cost here)
            lr = To7(px[loIdx].R, p0); lg = To7(px[loIdx].G, p0); lb = To7(px[loIdx].B, p0);
            hr = To7(px[hiIdx].R, p1); hg = To7(px[hiIdx].G, p1); hb = To7(px[hiIdx].B, p1);
        }
        ...
        if (ComputeLs3D(w, px, Bc7Tables.LsTab4, totR, totG, totB, xl, xh))
        {
            ...
            {
                p0 = 1; p1 = 1;
                lr = To7(RoundPos(xl[0]), p0); lg = To7(RoundPos(xl[1]), p0); lb = To7(RoundPos(xl[2]), p0);
                hr = To7(RoundPos(xh[0]), p1); hg = To7(RoundPos(xh[1]), p1); hb = To7(RoundPos(xh[2]), p1);
            }
        }
        trueSse = 0;
        ...
        else
        {
            ...
            if (wantSse)
                trueSse = (uint)SseSingleSubsetRgba(
                    From7(lr, p0), From7(lg, p0), From7(lb, p0), From7(127, p0),
                    From7(hr, p1), From7(hg, p1), From7(hb, p1), From7(127, p1),
    ...
    private static void EvalWeightsMode6(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w,
        int lr, int lg, int lb, int hr, int hg, int hb, uint p0, uint p1)
    {
        lr = From7(lr, p0); lg = From7(lg, p0); lb = From7(lb, p0);
        hr = From7(hr, p1); hg = From7(hg, p1); hb = From7(hb, p1);
        int dr = hr - lr, dg = hg - lg, db = hb - lb;
        float f = 15.0f / (dr * dr + dg * dg + db * db + 0.00000125f);
    ...
    private static void EncodeMode6(scoped Span<byte> block,
        uint lr, uint lg, uint lb, uint la, uint p0,
        uint hr, uint hg, uint hb, uint ha, uint p1,
        scoped ReadOnlySpan<int> weights)
    {
        uint weightInv = 0;
        if ((weights[0] & 8) != 0) // anchor index MSB must be 0 -> swap endpoints + invert indices
        {
            (lr, hr) = (hr, lr); (lg, hg) = (hg, lg); (lb, hb) = (hb, lb); (la, ha) = (ha, la);
            (p0, p1) = (p1, p0);
            weightInv = 15;
        }

        ulong x = 0b1000000UL                        // mode 6 marker (six 0s then a 1)
                | ((ulong)lr << 7)  | ((ulong)hr << 14)
                | ((ulong)lg << 21) | ((ulong)hg << 28)
                | ((ulong)lb << 35) | ((ulong)hb << 42)
                | ((ulong)la << 49) | ((ulong)ha << 56);

        block[0] = (byte)x;         block[1] = (byte)(x >> 8);  block[2] = (byte)(x >> 16);
        block[3] = (byte)(x >> 24); block[4] = (byte)(x >> 32); block[5] = (byte)(x >> 40);
        block[6] = (byte)(x >> 48);
        ...
        block[8]  = (byte)y;         block[9]  = (byte)(y >> 8);  block[10] = (byte)(y >> 16);
        block[11] = (byte)(y >> 24); block[12] = (byte)(y >> 32); block[13] = (byte)(y >> 40);
        block[14] = (byte)(y >> 48); block[15] = (byte)(y >> 56);
    }
}
