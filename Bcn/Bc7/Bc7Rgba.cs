...
static partial class Bc7Block
{
    ...
    const int DpBlockVarThreshRgba = 1 * 16;
    const float AlphaDecorrThreshold = 0.995f;
    const float StrongDecorrThreshRgba = 0.85f;
    const int TrivialBlockThreshRgba = 2 * 16;
    const int MinBlockMaxVar23SubsetsRgba = 100 * 16;
    const float HighOrthoEnergyThreshRgba = 1.0f * 16;
    const float OrthoRatio23SubsetThreshRgba = 0.004f;

    static void PackRgbaAnalytical(scoped ReadOnlySpan<ColorRgba> px, scoped Span<byte> block, Bc7Flags flags)
    {
        ...
        var bmv4 = Math.Max(Math.Max(icov[0], icov[4]), Math.Max(icov[7], icov[9]));
        ...
        if ((flags & Bc7Flags.UseDualPlaneRgba) != 0 && bmv4 >= DpBlockVarThreshRgba)
        {
            ...
            if (icov[9] > 0)
            {
                var p03 = icov[0] != 0 ? MathF.Abs(icov[3] / MathF.Sqrt(rv * av)) : 1f;
                var p13 = icov[4] != 0 ? MathF.Abs(icov[6] / MathF.Sqrt(gv * av)) : 1f;
                var p23 = icov[7] != 0 ? MathF.Abs(icov[8] / MathF.Sqrt(bv * av)) : 1f;
        ...
        {
            PackMode6RgbaTrivialOrDefault(px, block, false, 0, 0, 0, 0, tR, tG, tB, tA, flags, false, out _);
            return;
        }

        Span<float> cov = stackalloc float[10];
        for (var i = 0; i < 10; i++) cov[i] = icov[i];
        EstimatePrincipalAxis4D(cov, bmv4, out var x1, out var y1, out var z1, out var w1);
        ...
        spans[2] = maxB - minB;
        spans[3] = maxA - minA;
        var mode6Slam = EstimateSlamToLineSse4D(cov, x1, y1, z1, w1, out var orthoRatio);
        var mode6Est = mode6Slam + AnalyticalQuantEstSse(128, 16, 4, spans, default, 1.0f, 16);

        float mode45Est = 1e9f, mode7Est = 1e9f;
        Span<byte> b45 = stackalloc byte[16];
        Span<byte> b7 = stackalloc byte[16];
        var m45 = dpChan >= 0 && PackMode4Or5(b45, px, dpChan, mode6Est, flags, false, out mode45Est, out _);
        ...
        if (mode45Est < mode7Est && mode45Est < mode6Est)
        {
            b45.CopyTo(block);
            return;
        }

        if (mode7Est < mode45Est && mode7Est < mode6Est)
        {
            b7.CopyTo(block);
            return;
        }

        PackMode6RgbaTrivialOrDefault(px, block, true, x1, y1, z1, w1, tR, tG, tB, tA, flags, false, out _);
    }
    ...
    static void PackRgbaPartialAnalytical(scoped ReadOnlySpan<ColorRgba> px, scoped Span<byte> block, Bc7Flags flags)
    {
        var fc = AsU32(in px[0]);
        var solid = true;
        for (var k = 1; k < 16; k++)
        ...
        PackMode6RgbaTrivialOrDefault(px, block, true, x1, y1, z1, w1, tR, tG, tB, tA, flags, true, out var mode6Sse);
        ...
        if (mode6Sse != 0)
        {
            ...
            {
                if (PackMode7Rgba(b7, px, x1, y1, z1, w1, meanR, meanG, meanB, meanA, 1e9f, flags, true, out _, out var t7)) sse7 = t7;
            }

            if (dpChan >= 0 && PackMode4Or5(b45, px, dpChan, 1e9f, flags, true, out _, out var t45)) sse45 = t45;

            var best = Math.Min(mode6Sse, Math.Min(sse45, sse7));
    ...
    static void PackMode6RgbaTrivialOrDefault(scoped ReadOnlySpan<ColorRgba> px,
    ...
    static void EvalWeightsMode6Rgba(scoped ReadOnlySpan<ColorRgba> px,
        scoped Span<int> w,
        int lr,
        int lg,
        int lb,
        ...
        uint p1)
    {
        ...
        var f = 15.0f / (dr * dr + dg * dg + db * db + da * da + 0.00000125f);
        var sofs = -(lr * dr + lg * dg + lb * db + la * da);
        EvalSelRgba(px, w, dr, dg, db, da, sofs, f, 15);
    }

    static void EstimatePrincipalAxis4D(scoped ReadOnlySpan<float> cov,
        int bmv,
        out float x1,
        out float y1,
        out float z1,
        out float w1)
    {
        ...
        for (var it = 0; it < 4; it++)
        {
            ...
            var t = MathF.Sqrt(x1 * x1 + y1 * y1 + z1 * z1 + w1 * w1);
            if (t > SmallFloat)
            {
                t = 1f / t;
                x1 *= t;
    ...
    static float EstimateSlamToLineSse4D(scoped ReadOnlySpan<float> cov, float xr, float yr, float zr, float wr, out float orthoRatio)
    {
        var totalVar = cov[0] + cov[4] + cov[7] + cov[9];
        var l = MathF.Sqrt(xr * xr + yr * yr + zr * zr + wr * wr);
        if (l < SmallFloat)
        {
            xr = yr = zr = wr = 0.5f;
        }
        ...
        var xr2 = cov[0] * xr + cov[1] * yr + cov[2] * zr + cov[3] * wr;
        var xg2 = cov[1] * xr + cov[4] * yr + cov[5] * zr + cov[6] * wr;
        var xb2 = cov[2] * xr + cov[5] * yr + cov[7] * zr + cov[8] * wr;
        var xa2 = cov[3] * xr + cov[6] * yr + cov[8] * zr + cov[9] * wr;
        var pav = xr2 * xr + xg2 * yr + xb2 * zr + xa2 * wr;
        var ov = MathF.Max(0f, totalVar - pav);
