...
internal static partial class Bc7Block
{
    ...
    private static int Dist3(int r, int g, int b, int r2, int g2, int b2)
    { int dr = r - r2, dg = g - g2, db = b - b2; return dr * dr + dg * dg + db * db; }

    // Build a 3-subset assignment from the block axis: split by sign, then re-split the worse-fit half.
    private static bool Determine3Subsets(scoped Span<byte> final3, scoped ReadOnlySpan<ColorRgba> px,
        float axR, float axG, float axB, int meanR, int meanG, int meanB)
    {
        Span<int> sub = stackalloc int[16];
        Span<int> mR = stackalloc int[2]; Span<int> mG = stackalloc int[2]; Span<int> mB = stackalloc int[2];
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            int s = (rd * axR + gd * axG + bd * axB) > 0f ? 1 : 0;
            sub[i] = s; mR[s] += _qq0.R; mG[s] += _qq0.G; mB[s] += _qq0.B; tot[s]++;
        }
        for (int s = 0; s < 2; s++)
        {
            int t = tot[s]; if (t == 0) return false;
            int h = t >> 1; mR[s] = (mR[s] + h) / t; mG[s] = (mG[s] + h) / t; mB[s] = (mB[s] + h) / t;
        }
        Span<int> sse = stackalloc int[2];
        ref ColorRgba _pp1 = ref MemoryMarshal.GetReference(px);
        for (int i = 0; i < 16; i++) {
            ref readonly ColorRgba _qq1 = ref Unsafe.Add(ref _pp1, i); int s = sub[i]; sse[s] += Dist3(_qq1.R, _qq1.G, _qq1.B, mR[s], mG[s], mB[s]); }

        int split = sse[1] > sse[0] ? 1 : 0;
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            int y = ((_qq2.R + _qq2.G + _qq2.B) << 4) + i;
        ...
        for (int i = 0; i < 16; i++)
            ...
            ref readonly ColorRgba _qq3 = ref Unsafe.Add(ref _pp3, i);
                int d0 = Dist3(lr, lg, lb, _qq3.R, _qq3.G, _qq3.B);
                int d1 = Dist3(hr, hg, hb, _qq3.R, _qq3.G, _qq3.B);
    ...
    private static int Pick3SubsetPatIndex(scoped ReadOnlySpan<byte> desired, out int patFirst16)
    {
        patFirst16 = 0;
        int m0 = 0, m1 = 0, m2 = 0;
        for (int i = 0; i < 16; i++) { int s = desired[i]; if (s == 0) m0 |= 1 << i; else if (s == 1) m1 |= 1 << i; else m2 |= 1 << i; }
        int n0 = BitOperations.PopCount((uint)m0), n1 = BitOperations.PopCount((uint)m1), n2 = 16 - n0 - n1;

        int bestScore = -1, bestPat = 0;
        for (int p = 0; p < 64; p++)
        {
            uint packed = Bc7Tables.Part3Bitmasks[p];
            ...
            int sc = s0; if (s1 > sc) sc = s1; if (s2 > sc) sc = s2; if (s3 > sc) sc = s3; if (s4 > sc) sc = s4; if (s5 > sc) sc = s5;
            if (sc > bestScore)
            {
                bestScore = sc; bestPat = p;
                if (sc == 16) { if (p <= 15) patFirst16 = bestPat; break; }
            }
    ...
    private static bool PackMode0Or2Rgb(scoped Span<byte> block, scoped ReadOnlySpan<ColorRgba> px,
        float axR, float axG, float axB, int meanR, int meanG, int meanB,
        float sseEstToBeat, Bc7Flags flags, bool wantSse, out float estSse, out uint trueSse)
    {
        ...
        int patM2 = Pick3SubsetPatIndex(desired, out int patM0);   // patM0 <= 15, patM2 <= 63

        // per-mode (0,2) x per-subset (0,1,2) flattened accumulators
        Span<int> totC = stackalloc int[6];
        Span<int> loC = stackalloc int[6]; Span<int> hiC = stackalloc int[6];
        Span<int> totR = stackalloc int[6]; Span<int> totG = stackalloc int[6]; Span<int> totB = stackalloc int[6];
        ...
        Span<int> mR = stackalloc int[3]; Span<int> mG = stackalloc int[3]; Span<int> mB = stackalloc int[3];
        Span<int> ar = stackalloc int[3]; Span<int> ag = stackalloc int[3]; Span<int> ab = stackalloc int[3];
        Span<int> lowDot = stackalloc int[3]; Span<int> highDot = stackalloc int[3];
        ...
        for (int mode = 0; mode < 2; mode++)
        {
            ...
            if (mode == 1 && patM0 == patM2)
            {
                for (int s = 0; s < 3; s++)
                {
                    totC[3 + s] = totC[s]; loC[3 + s] = loC[s]; hiC[3 + s] = hiC[s];
                    totR[3 + s] = totR[s]; totG[3 + s] = totG[s]; totB[3 + s] = totB[s];
                }
                slamMode[1] = slamMode[0];
            }
            else
            {
                int pat = mode == 0 ? patM0 : patM2;
                for (int s = 0; s < 3; s++) { totC[bse + s] = 0; totR[bse + s] = 0; totG[bse + s] = 0; totB[bse + s] = 0; }
                ref ColorRgba _pp4 = ref MemoryMarshal.GetReference(px);
                for (int i = 0; i < 16; i++)
                {
                    ref readonly ColorRgba _qq4 = ref Unsafe.Add(ref _pp4, i);
                    int s = Bc7Tables.Partition3[pat * 16 + i];
                    totR[bse + s] += _qq4.R; totG[bse + s] += _qq4.G; totB[bse + s] += _qq4.B; totC[bse + s]++;
                }
                for (int s = 0; s < 3; s++)
                {
                    int t = totC[bse + s], h = t >> 1;
                    mR[s] = (totR[bse + s] + h) / t; mG[s] = (totG[bse + s] + h) / t; mB[s] = (totB[bse + s] + h) / t;
                }
                icov.Clear();
                ref ColorRgba _pp5 = ref MemoryMarshal.GetReference(px);
                for (int i = 0; i < 16; i++)
                {
                    ref readonly ColorRgba _qq5 = ref Unsafe.Add(ref _pp5, i);
                ...
                ref ColorRgba _pp6 = ref MemoryMarshal.GetReference(px);
                for (int i = 0; i < 16; i++)
                {
                    ref readonly ColorRgba _qq6 = ref Unsafe.Add(ref _pp6, i);
                    int s = Bc7Tables.Partition3[pat * 16 + i];
                    int dot = _qq6.R * ar[s] + _qq6.G * ag[s] + _qq6.B * ab[s] + i;
                    if (dot < lowDot[s]) lowDot[s] = dot;
                    if (dot > highDot[s]) highDot[s] = dot;
                }
                for (int s = 0; s < 3; s++) { loC[bse + s] = lowDot[s] & 15; hiC[bse + s] = highDot[s] & 15; }
            }
            ...
            for (int s = 0; s < 3; s++)
            {
                ...
                spans[0] = px[hc].R - px[lc].R; spans[1] = px[hc].G - px[lc].G; spans[2] = px[hc].B - px[lc].B;
        ...
        Span<uint> lr = stackalloc uint[3]; Span<uint> lg = stackalloc uint[3]; Span<uint> lb = stackalloc uint[3];
        Span<uint> hr = stackalloc uint[3]; Span<uint> hg = stackalloc uint[3]; Span<uint> hb = stackalloc uint[3];
        ...
        Span<int> eLoR = stackalloc int[3]; Span<int> eLoG = stackalloc int[3]; Span<int> eLoB = stackalloc int[3];
        Span<int> eHiR = stackalloc int[3]; Span<int> eHiG = stackalloc int[3]; Span<int> eHiB = stackalloc int[3];
        ...
        if (est0 < est2)
        {
            ...
            for (int s = 0; s < 3; s++)
            {
                ...
                else
                {
                    ...
                    pbits[s * 2] = lp; lr[s] = (uint)To4(px[lc].R, lp); lg[s] = (uint)To4(px[lc].G, lp); lb[s] = (uint)To4(px[lc].B, lp);
                    uint hp = (uint)((px[hc].R + px[hc].G + px[hc].B) >= 129 ? 1 : 0);
                    pbits[s * 2 + 1] = hp; hr[s] = (uint)To4(px[hc].R, hp); hg[s] = (uint)To4(px[hc].G, hp); hb[s] = (uint)To4(px[hc].B, hp);
                }
            }
            ...
            for (int s = 0; s < 3; s++)
            {
                ...
                else
                {
                    ...
                    uint lp=(uint)((l0+l1+l2)>=129?1:0); pbits[s*2]=lp; lr[s]=(uint)To4(l0,lp); lg[s]=(uint)To4(l1,lp); lb[s]=(uint)To4(l2,lp);
                    uint hp=(uint)((h0+h1+h2)>=129?1:0); pbits[s*2+1]=hp; hr[s]=(uint)To4(h0,hp); hg[s]=(uint)To4(h1,hp); hb[s]=(uint)To4(h2,hp);
                }
            }
            EvalWeightsMode0(px, w, lr, lg, lb, hr, hg, hb, pbits, (uint)patM0);
            for (int s = 0; s < 3; s++) {
        ...
        else
        {
            ...
            for (int s = 0; s < 3; s++)
            {
                ...
                lr[s] = (uint)To5(px[lc].R); lg[s] = (uint)To5(px[lc].G); lb[s] = (uint)To5(px[lc].B);
                hr[s] = (uint)To5(px[hc].R); hg[s] = (uint)To5(px[hc].G); hb[s] = (uint)To5(px[hc].B);
            }
            ...
            for (int s = 0; s < 3; s++)
            {
                ...
                lr[s] = (uint)To5(RoundPos(xl[0])); lg[s] = (uint)To5(RoundPos(xl[1])); lb[s] = (uint)To5(RoundPos(xl[2]));
                hr[s] = (uint)To5(RoundPos(xh[0])); hg[s] = (uint)To5(RoundPos(xh[1])); hb[s] = (uint)To5(RoundPos(xh[2]));
            }
            EvalWeightsMode2(px, w, lr, lg, lb, hr, hg, hb, (uint)patM2);
            for (int s = 0; s < 3; s++) {
    ...
    private static void EvalWeightsMode0(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w,
        ...
        scoped ReadOnlySpan<uint> pbits, uint pat)
    {
        Span<int> dr = stackalloc int[3]; Span<int> dg = stackalloc int[3]; Span<int> db = stackalloc int[3];
        Span<float> f = stackalloc float[3]; Span<int> sofs = stackalloc int[3];
        Span<int> lr = stackalloc int[3]; Span<int> lg = stackalloc int[3]; Span<int> lb = stackalloc int[3];
        for (int s = 0; s < 3; s++)
        {
            lr[s] = From4((int)blr[s], pbits[s * 2]); lg[s] = From4((int)blg[s], pbits[s * 2]); lb[s] = From4((int)blb[s], pbits[s * 2]);
    ...
    private static void EvalWeightsMode2(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w,
        scoped ReadOnlySpan<uint> blr, scoped ReadOnlySpan<uint> blg, scoped ReadOnlySpan<uint> blb,
        scoped ReadOnlySpan<uint> bhr, scoped ReadOnlySpan<uint> bhg, scoped ReadOnlySpan<uint> bhb, uint pat)
    {
        Span<int> dr = stackalloc int[3]; Span<int> dg = stackalloc int[3]; Span<int> db = stackalloc int[3];
        Span<float> f = stackalloc float[3]; Span<int> sofs = stackalloc int[3];
        Span<int> lr = stackalloc int[3]; Span<int> lg = stackalloc int[3]; Span<int> lb = stackalloc int[3];
        for (int s = 0; s < 3; s++)
        {
            lr[s] = From5((int)blr[s]); lg[s] = From5((int)blg[s]); lb[s] = From5((int)blb[s]);
    ...
    private static void EncodeMode0(scoped Span<byte> block, uint partId,
        ...
        scoped Span<uint> p, scoped ReadOnlySpan<int> weights)
    {
        ...
        if ((weights[0] & 4) != 0) { (lr[0], hr[0]) = (hr[0], lr[0]); (lg[0], hg[0]) = (hg[0], lg[0]); (lb[0], hb[0]) = (hb[0], lb[0]); (p[0], p[1]) = (p[1], p[0]); winv[0] = 7; }
        if ((weights[a0] & 4) != 0) { (lr[1], hr[1]) = (hr[1], lr[1]); (lg[1], hg[1]) = (hg[1], lg[1]); (lb[1], hb[1]) = (hb[1], lb[1]); (p[2], p[3]) = (p[3], p[2]); winv[1] = 7; }
        if ((weights[a1] & 4) != 0) { (lr[2], hr[2]) = (hr[2], lr[2]); (lg[2], hg[2]) = (hg[2], lg[2]); (lb[2], hb[2]) = (hb[2], lb[2]); (p[4], p[5]) = (p[5], p[4]); winv[2] = 7; }

        ulong low = 1UL | ((ulong)partId << 1)
        ...
        ulong high = ((ulong)lb[1] >> 3) | ((ulong)hb[1] << 1) | ((ulong)lb[2] << 5) | ((ulong)hb[2] << 9)
    ...
    private static void EncodeMode2(scoped Span<byte> block, uint partId,
        scoped Span<uint> lr, scoped Span<uint> lg, scoped Span<uint> lb, scoped Span<uint> hr, scoped Span<uint> hg, scoped Span<uint> hb,
        scoped ReadOnlySpan<int> weights)
    {
        int a0 = Bc7Tables.AnchorThird1[(int)partId], a1 = Bc7Tables.AnchorThird2[(int)partId];
        Span<uint> winv = stackalloc uint[3];
        if ((weights[0] & 2) != 0) { (lr[0], hr[0]) = (hr[0], lr[0]); (lg[0], hg[0]) = (hg[0], lg[0]); (lb[0], hb[0]) = (hb[0], lb[0]); winv[0] = 3; }
        if ((weights[a0] & 2) != 0) { (lr[1], hr[1]) = (hr[1], lr[1]); (lg[1], hg[1]) = (hg[1], lg[1]); (lb[1], hb[1]) = (hb[1], lb[1]); winv[1] = 3; }
        if ((weights[a1] & 2) != 0) { (lr[2], hr[2]) = (hr[2], lr[2]); (lg[2], hg[2]) = (hg[2], lg[2]); (lb[2], hb[2]) = (hb[2], lb[2]); winv[2] = 3; }

        ulong v = 0b100UL | ((ulong)partId << 3)
        ...
        block[8] = (byte)v1; block[9] = (byte)(v1 >> 8); block[10] = (byte)(v1 >> 16); block[11] = (byte)(v1 >> 24);
        v1 >>= 32;
        int ofs = 3;
        for (int i = 0; i < 16; i++)
        {
        ...
        block[12] = (byte)v1; block[13] = (byte)(v1 >> 8); block[14] = (byte)(v1 >> 16); block[15] = (byte)(v1 >> 24);
    }
}