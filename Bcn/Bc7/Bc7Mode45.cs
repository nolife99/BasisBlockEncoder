...
{
    static bool PackMode4Or5(scoped Span<byte> block,
        scoped ReadOnlySpan<ColorRgba> origPx,
        int dpChan,
        float sseEstToBeat,
        Bc7Flags flags,
        bool wantSse,
        ...
        out uint trueSse)
    {
        ...
        var bmv = Math.Max(icov[0], Math.Max(icov[3], icov[5]));
        Span<float> cov = stackalloc float[6];
        for (var i = 0; i < 6; i++) cov[i] = icov[i];
        EstimatePrincipalAxis(cov, bmv, out var ax, out var ay, out var az);
        var rgbSlam = EstimateSlamToLineSse(cov, ax, ay, az, out _);

        int sr = 306, sg = 601, sb = 117;
        var kk = MathF.Max(MathF.Abs(ax), MathF.Max(MathF.Abs(ay), MathF.Abs(az)));
        if (kk >= SmallFloat)
        {
            var m = 2048f / kk;
            sr = (int)(ax * m);
        ...
        for (var i = 0; i < 16; i++)
        {
            ...
            var dot = _qq2.R * sr + _qq2.G * sg + _qq2.B * sb + i;
        ...
        rgbSpans[0] = px[hic].R - px[loc].R;
        rgbSpans[1] = px[hic].G - px[loc].G;
        rgbSpans[2] = px[hic].B - px[loc].B;
        var aSpan = maxA - minA;

        var m4r3a2 = rgbSlam + AnalyticalQuantEstSse(32, 8, 3, rgbSpans, default, 1f, 16) + AnalyticalQuantEstSse(64, 4, aSpan, 1f, 1f, 16);
        var m4r2a3 = rgbSlam + AnalyticalQuantEstSse(32, 4, 3, rgbSpans, default, 1f, 16) + AnalyticalQuantEstSse(64, 8, aSpan, 1f, 1f, 16);
        var m5 = rgbSlam + AnalyticalQuantEstSse(128, 4, 3, rgbSpans, default, 1f, 16) + AnalyticalQuantEstSse(256, 4, aSpan, 1f, 1f, 16);

        var rot = (uint)(dpChan + 1 & 3);
        Span<int> w0 = stackalloc int[16];
        ...
        if (m5 < MathF.Min(m4r3a2, m4r2a3))
        {
            ...
            {
                lr = RoundPos(xl[0] * 127f / 255f);
                lg = RoundPos(xl[1] * 127f / 255f);
                lb = RoundPos(xl[2] * 127f / 255f);
                hr = RoundPos(xh[0] * 127f / 255f);
                hg = RoundPos(xh[1] * 127f / 255f);
                hb = RoundPos(xh[2] * 127f / 255f);
                EvalRgbPlane(px, w0, From7(lr), From7(lg), From7(lb), From7(hr), From7(hg), From7(hb), 4);
            }

            EvalAPlane(px, w1, la, ha, 4);
            if (ComputeLs1D(w1, px, 3, Bc7Tables.LsTab2, totA, out nal, out nah))
            {
                la = RoundPos(nal);
                ha = RoundPos(nah);
                EvalAPlane(px, w1, la, ha, 4);
            ...
            if (wantSse)
            {
                var sx = SseDualPlane(From7(lr), From7(lg), From7(lb), la, From7(hr), From7(hg), From7(hb), ha, Bc7Tables.Weights2, Bc7Tables.Weights2, w0, w1, px);
        ...
        else if (m4r3a2 < m4r2a3)
        {
            ...
            {
                lr = RoundPos(xl[0] * 31f / 255f);
                lg = RoundPos(xl[1] * 31f / 255f);
                lb = RoundPos(xl[2] * 31f / 255f);
                hr = RoundPos(xh[0] * 31f / 255f);
                hg = RoundPos(xh[1] * 31f / 255f);
                hb = RoundPos(xh[2] * 31f / 255f);
            ...
            if (ComputeLs1D(w1, px, 3, Bc7Tables.LsTab2, totA, out nal, out nah))
            {
                la = RoundPos(nal * 63f / 255f);
                ha = RoundPos(nah * 63f / 255f);
                EvalAPlane(px, w1, From6(la), From6(ha), 4);
            }

            if (wantSse)
            {
                var sx = SseDualPlane(From5(lr), From5(lg), From5(lb), From6(la), From5(hr), From5(hg), From5(hb), From6(ha), Bc7Tables.Weights3, Bc7Tables.Weights2, w0, w1, px);
                trueSse = sx > uint.MaxValue ? uint.MaxValue : (uint)sx;
            }
        ...
        else
        {
            ...
            if (ComputeLs1D(w1, px, 3, Bc7Tables.LsTab3, totA, out nal, out nah))
            {
                la = RoundPos(nal * 63f / 255f);
                ha = RoundPos(nah * 63f / 255f);
                EvalAPlane(px, w1, From6(la), From6(ha), 8);
            }

            if (wantSse)
            {
                var sx = SseDualPlane(From5(lr), From5(lg), From5(lb), From6(la), From5(hr), From5(hg), From5(hb), From6(ha), Bc7Tables.Weights2, Bc7Tables.Weights3, w0, w1, px);
                trueSse = sx > uint.MaxValue ? uint.MaxValue : (uint)sx;
            }
    ...
    static void EvalRgbPlane(scoped ReadOnlySpan<ColorRgba> px,
        scoped Span<int> w,
        int lr,
        int lg,
        int lb,
        ...
        int levels)
    {
        ...
        var f = (levels - 1) / (dr * dr + dg * dg + db * db + 0.00000125f);
        var sofs = lr * dr + lg * dg + lb * db;
        EvalSelRgb(px, w, dr, dg, db, -sofs, f, mx);
    }

    // Scalar-plane (rotated channel, stored in .A) index eval; 1-D.
    static void EvalAPlane(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w, int la, int ha, int levels)
    {
        int da = ha - la, mx = levels - 1;
        var f = (levels - 1) / (da + 0.00000125f);
        EvalSelA(px, w, la, f, mx);
    }

    static void EncodeMode5(scoped Span<byte> block,
        int lr,
        int lg,
        int lb,
        int la,
        ...
        int hb,
        int ha,
        scoped ReadOnlySpan<int> colW,
        scoped ReadOnlySpan<int> alpW,
        uint rot)
    {
        ...
        var high = (ulong)Ha >> 6 & 3;
        var ofs = 2;
        for (var i = 0; i < 16; i++)
        {
            ulong w = (uint)(colW[i] ^ (int)colorInv);
            high |= w << ofs;
            ofs += 2 - (i == 0 ? 1 : 0);
        }
        ...
        for (var i = 0; i < 16; i++)
        {
            ulong w = (uint)(alpW[i] ^ (int)alphaInv);
            high |= w << ofs;
            ofs += 2 - (i == 0 ? 1 : 0);
        }
    ...
    static void EncodeMode4(scoped Span<byte> block,
        int lr,
        int lg,
        int lb,
        int la,
        ...
        uint flag)
    {
        uint Lr = (uint)lr, Lg = (uint)lg, Lb = (uint)lb, La = (uint)la, Hr = (uint)hr, Hg = (uint)hg, Hb = (uint)hb, Ha = (uint)ha;
        uint inv0 = 0, inv1 = 0;
        var p2 = flag != 0 ? w1 : w0; // 2-bit plane
        var p3 = flag != 0 ? w0 : w1; // 3-bit plane
        ...
        var x = Lr | Hr << 5 | Lg << 10 | Hg << 15 | Lb << 20 | Hb << 25 | (ulong)La << 30 | (ulong)Ha << 36;
        block[1] = (byte)x;
        block[2] = (byte)(x >> 8);
        block[3] = (byte)(x >> 16);
        block[4] = (byte)(x >> 24);
