...
internal static partial class Bc7Block
{
    internal static void Encode(scoped ReadOnlySpan<ColorRgba> pixels, scoped Span<byte> output, Bc7Flags flags)
    {
        ...
        ref uint pu = ref MemoryMarshal.GetReference(MemoryMarshal.Cast<ColorRgba, uint>(pixels));
        bool hasAlpha;
        if (Vector256.IsHardwareAccelerated)
        {
            Vector256<uint> top = Vector256.Create(0xFF000000u);
            hasAlpha = !(Vector256.EqualsAll(Vector256.LoadUnsafe(ref pu, 0) & top, top)
                      && Vector256.EqualsAll(Vector256.LoadUnsafe(ref pu, 8) & top, top));
        }
        else
        {
            hasAlpha = false;
            for (nuint i = 0; i < 16; i++) if ((Unsafe.Add(ref pu, i) & 0xFF000000u) != 0xFF000000u) { hasAlpha = true; break; }
        }
        if (hasAlpha)
        {
    ...
    private const int   TrivialBlockThreshRgb    = 20 * 16;   // below this variance: skip PCA, use trivial mode 6
    private const int   DpBlockVarThresh         = 2 * 16;    // consider dual plane above this
    private const float StrongCorrThresh         = 0.85f;     // channel-decorrelation cutoff for dual plane
    private const int   MinBlockMaxVar23Subsets  = 100 * 16;  // consider 2/3 subsets above this
    private const float OrthoRatio23SubsetThresh = 0.004f;    // ...and only if this fraction of energy is off-axis
    private const float HighOrthoEnergyThresh    = 1.0f * 16; // gate for actually attempting 2+ subsets
    private const int   MinBlockMaxVar3Subsets   = 500 * 16;  // consider 3 subsets above this
    private const float SmallFloat               = 1e-7f;     // basisu::SMALL_FLOAT_VAL
    ...
    private const long Mode6PruneMargin = 32;             // >= mean-rounding inflation (<=12) + fp slack; keeps the bound sound

    // Reinterpret a 4-byte ColorRgba as uint (== R|G<<8|B<<16|A<<24). net7-safe; replaces Unsafe.BitCast (net8+).
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static uint AsU32(in ColorRgba c) => Unsafe.As<ColorRgba, uint>(ref Unsafe.AsRef(in c));

    private static void PackRgbAnalytical(scoped ReadOnlySpan<ColorRgba> px, scoped Span<byte> block, Bc7Flags flags)
    {
        // solid fast path: all 16 pixels identical as uint
        uint fc = AsU32(in px[0]);
        bool solid = true;
        for (int k = 1; k < 16; k++)
            if (AsU32(in px[k]) != fc) { solid = false; break; }
        ...
        int blockMaxVar = Math.Max(icov[0], Math.Max(icov[3], icov[5]));
        if (blockMaxVar == 0) { PackMode5Solid(block, px[0]); return; }
        ...
        if ((flags & Bc7Flags.UseDualPlaneRgb) != 0 && blockMaxVar >= DpBlockVarThresh)
        {
            ...
            int active = (hasR ? 1 : 0) + (hasG ? 1 : 0) + (hasB ? 1 : 0);
            if (active >= 2)
            {
                ...
                float gb = (hasG && hasB) ? MathF.Abs(icov[4] / MathF.Sqrt(gv * bv)) : 1f;
        ...
        EstimatePrincipalAxis(cov, blockMaxVar, out float axR, out float axG, out float axB);

        // (2) slam-to-line residual + ortho ratio, and mode 6's predicted SSE (the baseline to beat).
        Span<int> spans = stackalloc int[4];
        spans[0] = maxR - minR; spans[1] = maxG - minG; spans[2] = maxB - minB; spans[3] = 0;
        bool needEst = ((flags & Bc7Flags.Use2SubsetsRgb) != 0) || (desiredDpChan >= 0);
            ...
            mode6Slam >= HighOrthoEnergyThresh)
        {
            ...
            {
                if (PackMode0Or2Rgb(block, px, axR, axG, axB, meanR, meanG, meanB, mode6Est, flags, false, out float est02, out _))
                {
    ...
    private static void PackRgbPartialAnalytical(scoped ReadOnlySpan<ColorRgba> px, scoped Span<byte> block, Bc7Flags flags)
    {
        uint fc = AsU32(in px[0]);
        bool solid = true;
        for (int k = 1; k < 16; k++)
        ...
        float mode6Slam = EstimateSlamToLineSse(cov, axR, axG, axB, out float orthoRatio);

        Span<byte> cand02 = stackalloc byte[16]; uint sse02 = uint.MaxValue;
        Span<byte> cand13 = stackalloc byte[16]; uint sse13 = uint.MaxValue;
        Span<byte> cand45 = stackalloc byte[16]; uint sse45 = uint.MaxValue;
            ...
            mode6Slam >= HighOrthoEnergyThresh)
        {
            if ((flags & Bc7Flags.Use3SubsetsRgb) != 0 && blockMaxVar >= MinBlockMaxVar3Subsets)
            {
                if (PackMode0Or2Rgb(cand02, px, axR, axG, axB, meanR, meanG, meanB, 1e9f, flags, true, out _, out uint t02)) sse02 = t02;
                if (PackMode1Or3Rgb(cand13, px, axR, axG, axB, meanR, meanG, meanB, 1e9f, flags, true, out _, out uint t13)) sse13 = t13;
            }
            else
            {
                if (PackMode1Or3Rgb(cand13, px, axR, axG, axB, meanR, meanG, meanB, 1e9f, flags, true, out _, out uint t13)) sse13 = t13;
        ...
        if (desiredDpChan >= 0) { if (PackMode4Or5(cand45, px, desiredDpChan, 1e9f, flags, true, out _, out uint t45)) sse45 = t45; }

        uint candBest = Math.Min(sse45, Math.Min(sse02, sse13));
        ...
        PackMode6Default(px, block, axR, axG, axB, totR, totG, totB, flags, true, out uint mode6Sse);
        if (mode6Sse == 0) return; // perfect mode 6 wins (replicates the original mode6Sse != 0 guard)
        uint best = Math.Min(mode6Sse, candBest);
    ...
    private static void PackTrivialMode6(scoped ReadOnlySpan<ColorRgba> px, scoped Span<byte> block, Bc7Flags flags)
    {
        ...
        EncodeMode6(block, (uint)lr, (uint)lg, (uint)lb, 127, p0,
                           (uint)hr, (uint)hg, (uint)hb, 127, p1, w);
    }
    ...
    private static void EstimatePrincipalAxis(scoped ReadOnlySpan<float> cov, int blockMaxVar,
        out float axR, out float axG, out float axB)
    {
        float sc = blockMaxVar != 0 ? 1.0f / blockMaxVar : 0f;
    ...
    internal static float MaxEig3x3Sym(scoped ReadOnlySpan<float> cov)
    {
        ...
        float p2 = aq * aq + dq * dq + fq * fq + 2f * p1;
        float p = MathF.Sqrt(p2 * (1f / 6f));
        float ip = 1f / p;
        ...
        float r = detB * 0.5f;
        r = r < -1f ? -1f : (r > 1f ? 1f : r);
        float phi = MathF.Acos(r) * (1f / 3f);
        float lam = q + 2f * p * MathF.Cos(phi);          // largest of the three eigenvalues
    ...
    private static float EstimateSlamToLineSse(scoped ReadOnlySpan<float> cov, float xr, float yr, float zr, out float orthoRatio)
    {
    ...
    private static double LambdaMax3x3Sym(double a, double b, double c, double d, double e, double f)
    {
        double p1 = b * b + c * c + e * e;
        if (p1 <= 0.0) return Math.Max(a, Math.Max(d, f)); // already diagonal
        double q = (a + d + f) / 3.0;
        double aq = a - q, dq = d - q, fq = f - q;
        double p = Math.Sqrt((aq * aq + dq * dq + fq * fq + 2.0 * p1) / 6.0);
        double ip = 1.0 / p;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int To7(int c8, uint pbit)
    {
        uint e = (uint)(c8 + (int)(pbit ^ 1)) >> 1;
        return (int)Math.Min(127u, e);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int From7(int v, uint p) => (v << 1) | (int)p;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int LerpC(int a, int b, int w) => (a * (64 - w) + b * w + 32) >> 6;
    ...
    internal static long SseMultiSubsetRgb(
        scoped ReadOnlySpan<int> loR, scoped ReadOnlySpan<int> loG, scoped ReadOnlySpan<int> loB,
        scoped ReadOnlySpan<int> hiR, scoped ReadOnlySpan<int> hiG, scoped ReadOnlySpan<int> hiB,
        scoped ReadOnlySpan<byte> part, int partBase, scoped ReadOnlySpan<int> wt,
        scoped ReadOnlySpan<int> sel, scoped ReadOnlySpan<ColorRgba> px)
    {
        Span<ColorRgba> dec = stackalloc ColorRgba[16];
        ref ColorRgba d = ref MemoryMarshal.GetReference(dec);
        ref int se = ref MemoryMarshal.GetReference(sel);
        ref int wtab = ref MemoryMarshal.GetReference(wt);
        ref byte pt = ref MemoryMarshal.GetReference(part);
        for (int i = 0; i < 16; i++)
        {
            int sub = Unsafe.Add(ref pt, partBase + i);
            int w = Unsafe.Add(ref wtab, Unsafe.Add(ref se, i));
            Unsafe.Add(ref d, i) = new ColorRgba((byte)LerpC(loR[sub], hiR[sub], w), (byte)LerpC(loG[sub], hiG[sub], w), (byte)LerpC(loB[sub], hiB[sub], w), 255);
        }
    ...
    internal static long SseMultiSubsetRgba(
        scoped ReadOnlySpan<int> loR, scoped ReadOnlySpan<int> loG, scoped ReadOnlySpan<int> loB, scoped ReadOnlySpan<int> loA,
        scoped ReadOnlySpan<int> hiR, scoped ReadOnlySpan<int> hiG, scoped ReadOnlySpan<int> hiB, scoped ReadOnlySpan<int> hiA,
        scoped ReadOnlySpan<byte> part, int partBase, scoped ReadOnlySpan<int> wt,
        scoped ReadOnlySpan<int> sel, scoped ReadOnlySpan<ColorRgba> px)
    {
        Span<ColorRgba> dec = stackalloc ColorRgba[16];
        ...
        for (int i = 0; i < 16; i++)
        {
            int sub = Unsafe.Add(ref pt, partBase + i);
            int w = Unsafe.Add(ref wtab, Unsafe.Add(ref se, i));
            Unsafe.Add(ref d, i) = new ColorRgba((byte)LerpC(loR[sub], hiR[sub], w), (byte)LerpC(loG[sub], hiG[sub], w), (byte)LerpC(loB[sub], hiB[sub], w), (byte)LerpC(loA[sub], hiA[sub], w));
        }
        return SsdRgba(px, dec);
    }
    ...
    internal static long SseDualPlane(
        ...
        scoped ReadOnlySpan<int> colourWt, scoped ReadOnlySpan<int> alphaWt,
        scoped ReadOnlySpan<int> w0, scoped ReadOnlySpan<int> w1, scoped ReadOnlySpan<ColorRgba> px)
    {
        Span<ColorRgba> dec = stackalloc ColorRgba[16];
        ...
        {
            ref ColorRgba d = ref MemoryMarshal.GetReference(dec);
            ref int cw = ref MemoryMarshal.GetReference(colourWt);
            ref int aw = ref MemoryMarshal.GetReference(alphaWt);
            ref int s0 = ref MemoryMarshal.GetReference(w0);
            ref int s1 = ref MemoryMarshal.GetReference(w1);
            for (int i = 0; i < 16; i++)
            {
                int wc = Unsafe.Add(ref cw, Unsafe.Add(ref s0, i));
                int wa = Unsafe.Add(ref aw, Unsafe.Add(ref s1, i));
                Unsafe.Add(ref d, i) = new ColorRgba((byte)LerpC(r0, r1, wc), (byte)LerpC(g0, g1, wc), (byte)LerpC(b0, b1, wc), (byte)LerpC(a0, a1, wa));
            }
        }
        return SsdRgba(px, dec);
    }
    ...
    private static void LerpDualPlaneX86(
        int r0, int g0, int b0, int a0, int r1, int g1, int b1, int a1,
        scoped ReadOnlySpan<int> colourWt, scoped ReadOnlySpan<int> alphaWt,
        scoped ReadOnlySpan<int> w0, scoped ReadOnlySpan<int> w1, scoped Span<ColorRgba> dec)
    {
        ref int cw = ref MemoryMarshal.GetReference(colourWt);
        ref int aw = ref MemoryMarshal.GetReference(alphaWt);
        ref int s0 = ref MemoryMarshal.GetReference(w0);
        ref int s1 = ref MemoryMarshal.GetReference(w1);
        ref byte db = ref Unsafe.As<ColorRgba, byte>(ref MemoryMarshal.GetReference(dec));
        Vector128<byte> aR = Vector128.Create((ushort)(((uint)r1 << 8) | (uint)r0)).AsByte();
        Vector128<byte> aG = Vector128.Create((ushort)(((uint)g1 << 8) | (uint)g0)).AsByte();
        Vector128<byte> aB = Vector128.Create((ushort)(((uint)b1 << 8) | (uint)b0)).AsByte();
        Vector128<byte> aA = Vector128.Create((ushort)(((uint)a1 << 8) | (uint)a0)).AsByte();
        Vector128<byte> c64 = Vector128.Create((byte)64);
        Vector128<short> c32 = Vector128.Create((short)32);
        for (int o = 0; o < 16; o += 8)
        {
            Vector128<byte> wcv = Vector128.Create(
                (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o)),     (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 1)),
                (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 2)), (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 3)),
                (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 4)), (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 5)),
                (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 6)), (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 7)),
                0, 0, 0, 0, 0, 0, 0, 0);
            Vector128<byte> wav = Vector128.Create(
                (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o)),     (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 1)),
                (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 2)), (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 3)),
                (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 4)), (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 5)),
                (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 6)), (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 7)),
                0, 0, 0, 0, 0, 0, 0, 0);
            Vector128<sbyte> bc = Sse2.UnpackLow(c64 - wcv, wcv).AsSByte();
            Vector128<sbyte> bal = Sse2.UnpackLow(c64 - wav, wav).AsSByte();
            Vector128<short> rR = Vector128.ShiftRightArithmetic(Ssse3.MultiplyAddAdjacent(aR, bc) + c32, 6);
            Vector128<short> rG = Vector128.ShiftRightArithmetic(Ssse3.MultiplyAddAdjacent(aG, bc) + c32, 6);
            Vector128<short> rB = Vector128.ShiftRightArithmetic(Ssse3.MultiplyAddAdjacent(aB, bc) + c32, 6);
            Vector128<short> rA = Vector128.ShiftRightArithmetic(Ssse3.MultiplyAddAdjacent(aA, bal) + c32, 6);
            WeaveStoreX86(rR, rG, rB, rA, ref db, o);
        }
    }
    ...
    private static void LerpDualPlaneArm(
        int r0, int g0, int b0, int a0, int r1, int g1, int b1, int a1,
        scoped ReadOnlySpan<int> colourWt, scoped ReadOnlySpan<int> alphaWt,
        scoped ReadOnlySpan<int> w0, scoped ReadOnlySpan<int> w1, scoped Span<ColorRgba> dec)
    {
        ...
        Vector64<byte> c64 = Vector64.Create((byte)64);
        Vector128<ushort> c32 = Vector128.Create((ushort)32);
        for (int o = 0; o < 16; o += 8)
        {
            Vector64<byte> wcv = Vector64.Create(
                (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o)),     (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 1)),
                (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 2)), (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 3)),
                (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 4)), (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 5)),
                (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 6)), (byte)Unsafe.Add(ref cw, Unsafe.Add(ref s0, o + 7)));
            Vector64<byte> wav = Vector64.Create(
                (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o)),     (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 1)),
                (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 2)), (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 3)),
                (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 4)), (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 5)),
                (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 6)), (byte)Unsafe.Add(ref aw, Unsafe.Add(ref s1, o + 7)));
            Vector64<byte> invc = AdvSimd.Subtract(c64, wcv);
            Vector64<byte> inva = AdvSimd.Subtract(c64, wav);
            Vector128<ushort> rR = AdvSimd.ShiftRightLogical(AdvSimd.Add(AdvSimd.MultiplyWideningLowerAndAdd(AdvSimd.MultiplyWideningLower(r0v, invc), r1v, wcv), c32), 6);
            Vector128<ushort> rG = AdvSimd.ShiftRightLogical(AdvSimd.Add(AdvSimd.MultiplyWideningLowerAndAdd(AdvSimd.MultiplyWideningLower(g0v, invc), g1v, wcv), c32), 6);
            Vector128<ushort> rB = AdvSimd.ShiftRightLogical(AdvSimd.Add(AdvSimd.MultiplyWideningLowerAndAdd(AdvSimd.MultiplyWideningLower(b0v, invc), b1v, wcv), c32), 6);
            Vector128<ushort> rA = AdvSimd.ShiftRightLogical(AdvSimd.Add(AdvSimd.MultiplyWideningLowerAndAdd(AdvSimd.MultiplyWideningLower(a0v, inva), a1v, wav), c32), 6);
            ...
            WeaveStoreArm(pR, pG, pB, pA, ref db, o);
        }
    }
    ...
    internal static long EvalSelSseRgb6X86(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w,
        int lr, int lg, int lb, int hr, int hg, int hb, uint p0, uint p1)
    {
        ...
        float f = 15.0f / (dr * dr + dg * dg + db * db + 0.00000125f);   // identical to EvalWeightsMode6
        int aofs = -(Lr * dr + Lg * dg + Lb * db);
        ...
        Vector256<int> loW = Vector256.Create(0, 4, 9, 13, 17, 21, 26, 30);     // Weights4[0..7]
        Vector256<int> hiW = Vector256.Create(34, 38, 43, 47, 51, 55, 60, 64);  // Weights4[8..15]
        ...
        for (nuint o = 0; o < 16; o += 8)
        {
            ...
            Vector256<int> R = (p & mask).AsInt32();
            Vector256<int> G = (Vector256.ShiftRightLogical(p, 8) & mask).AsInt32();
            Vector256<int> B = (Vector256.ShiftRightLogical(p, 16) & mask).AsInt32();
            Vector256<int> A = (Vector256.ShiftRightLogical(p, 24) & mask).AsInt32();
            ...
            Vector256<int> wt = Vector256.ConditionalSelect(Vector256.GreaterThanOrEqual(sel, eight),
                                    Vector256.Shuffle(hiW, sel - eight), Vector256.Shuffle(loW, sel));
            Vector256<int> iw = c64 - wt;
            Vector256<int> recR = Vector256.ShiftRightArithmetic(LrV * iw + HrV * wt + c32, 6);
            Vector256<int> recG = Vector256.ShiftRightArithmetic(LgV * iw + HgV * wt + c32, 6);
            Vector256<int> recB = Vector256.ShiftRightArithmetic(LbV * iw + HbV * wt + c32, 6);
            Vector256<int> recA = Vector256.ShiftRightArithmetic(LaV * iw + HaV * wt + c32, 6);
    ...
    internal static long EvalSelSseRgb2X86(scoped ReadOnlySpan<ColorRgba> px, scoped Span<int> w,
        scoped ReadOnlySpan<int> eLoR, scoped ReadOnlySpan<int> eLoG, scoped ReadOnlySpan<int> eLoB,
        scoped ReadOnlySpan<int> eHiR, scoped ReadOnlySpan<int> eHiG, scoped ReadOnlySpan<int> eHiB,
        int subsetMask, scoped ReadOnlySpan<int> wt, int mx)
    {
        int dr0 = eHiR[0] - eLoR[0], dg0 = eHiG[0] - eLoG[0], db0 = eHiB[0] - eLoB[0];
        int dr1 = eHiR[1] - eLoR[1], dg1 = eHiG[1] - eLoG[1], db1 = eHiB[1] - eLoB[1];
        float f0 = mx / (dr0 * dr0 + dg0 * dg0 + db0 * db0 + 0.00000125f);   // identical to EvalWeightsMode1/3
        float f1 = mx / (dr1 * dr1 + dg1 * dg1 + db1 * db1 + 0.00000125f);
        int so0 = eLoR[0] * dr0 + eLoG[0] * dg0 + eLoB[0] * db0;
        int so1 = eLoR[1] * dr1 + eLoG[1] * dg1 + eLoB[1] * db1;

        ref uint pru = ref MemoryMarshal.GetReference(MemoryMarshal.Cast<ColorRgba, uint>(px));
        ref int wr = ref MemoryMarshal.GetReference(w);
        Vector256<uint> m8 = Vector256.Create(0xFFu);
        ...
        Vector256<int> wtv = Vector256.Create(wt[0], wt[1], wt[2], wt[3],
            mx >= 4 ? wt[4] : 0, mx >= 5 ? wt[5] : 0, mx >= 6 ? wt[6] : 0, mx >= 7 ? wt[7] : 0);
        ...
        for (nuint o = 0; o < 16; o += 8)
        {
            ...
            Vector256<int> s0 = Avx.ConvertToVector256Int32WithTruncation(Fma.MultiplyAdd(Vector256.ConvertToSingle(R * dr0v + G * dg0v + B * db0v - so0v), f0v, halfv));
            Vector256<int> s1 = Avx.ConvertToVector256Int32WithTruncation(Fma.MultiplyAdd(Vector256.ConvertToSingle(R * dr1v + G * dg1v + B * db1v - so1v), f1v, halfv));
            Vector256<int> m = Vector256.Equals(Vector256.Create((subsetMask >> (int)o) & 0xFF) & bitsel, zero);   // true => subset 0
            Vector256<int> sel = Vector256.Min(Vector256.Max(Vector256.ConditionalSelect(m, s0, s1), zero), mxv);
            sel.StoreUnsafe(ref wr, o);
            Vector256<int> wsel = Vector256.Shuffle(wtv, sel);   // wt[sel], sel in 0..mx<=7
            Vector256<int> iw = c64 - wsel;
            Vector256<int> recR = Vector256.ConditionalSelect(m,
                Vector256.ShiftRightArithmetic(LoR0 * iw + HiR0 * wsel + c32, 6),
                Vector256.ShiftRightArithmetic(LoR1 * iw + HiR1 * wsel + c32, 6));
            Vector256<int> recG = Vector256.ConditionalSelect(m,
                Vector256.ShiftRightArithmetic(LoG0 * iw + HiG0 * wsel + c32, 6),
                Vector256.ShiftRightArithmetic(LoG1 * iw + HiG1 * wsel + c32, 6));
            Vector256<int> recB = Vector256.ConditionalSelect(m,
    ...
    internal static long SseSingleSubsetRgba(
        int loR, int loG, int loB, int loA, int hiR, int hiG, int hiB, int hiA,
        scoped ReadOnlySpan<int> wt, scoped ReadOnlySpan<int> sel, scoped ReadOnlySpan<ColorRgba> px)
    {
        Span<ColorRgba> dec = stackalloc ColorRgba[16];
        if (Ssse3.IsSupported)
        ...
        else
        {
            ...
            {
                int w = Unsafe.Add(ref wtab, Unsafe.Add(ref se, i));
                Unsafe.Add(ref d, i) = new ColorRgba(
                    (byte)LerpC(loR, hiR, w), (byte)LerpC(loG, hiG, w),
                    (byte)LerpC(loB, hiB, w), (byte)LerpC(loA, hiA, w));
            }
        }
        return SsdRgba(px, dec);
    }
    ...
    private static void LerpSingleSubsetX86(
        int loR, int loG, int loB, int loA, int hiR, int hiG, int hiB, int hiA,
        scoped ReadOnlySpan<int> wt, scoped ReadOnlySpan<int> sel, scoped Span<ColorRgba> dec)
    {
        ref int se = ref MemoryMarshal.GetReference(sel);
        ref int wtab = ref MemoryMarshal.GetReference(wt);
        ref byte db = ref Unsafe.As<ColorRgba, byte>(ref MemoryMarshal.GetReference(dec));
        Vector128<byte> aR = Vector128.Create((ushort)(((uint)hiR << 8) | (uint)loR)).AsByte();
        Vector128<byte> aG = Vector128.Create((ushort)(((uint)hiG << 8) | (uint)loG)).AsByte();
        Vector128<byte> aB = Vector128.Create((ushort)(((uint)hiB << 8) | (uint)loB)).AsByte();
        Vector128<byte> aA = Vector128.Create((ushort)(((uint)hiA << 8) | (uint)loA)).AsByte();
        Vector128<byte> c64 = Vector128.Create((byte)64);
        Vector128<short> c32 = Vector128.Create((short)32);
        for (int o = 0; o < 16; o += 8)
        {
            Vector128<byte> wv = Vector128.Create(
                (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o)),     (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 1)),
                (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 2)), (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 3)),
                (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 4)), (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 5)),
                (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 6)), (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 7)),
                0, 0, 0, 0, 0, 0, 0, 0);
            Vector128<sbyte> b = Sse2.UnpackLow(c64 - wv, wv).AsSByte();   // [64-w0,w0,64-w1,w1,...]
            Vector128<short> rR = Vector128.ShiftRightArithmetic(Ssse3.MultiplyAddAdjacent(aR, b) + c32, 6);
            Vector128<short> rG = Vector128.ShiftRightArithmetic(Ssse3.MultiplyAddAdjacent(aG, b) + c32, 6);
            Vector128<short> rB = Vector128.ShiftRightArithmetic(Ssse3.MultiplyAddAdjacent(aB, b) + c32, 6);
            Vector128<short> rA = Vector128.ShiftRightArithmetic(Ssse3.MultiplyAddAdjacent(aA, b) + c32, 6);
            WeaveStoreX86(rR, rG, rB, rA, ref db, o);
        }
    }

    // PACKUSWB the four channel results to bytes, then PUNPCK-weave R,G,B,A into RGBA AoS and store 8 px.
    private static void WeaveStoreX86(Vector128<short> rR, Vector128<short> rG, Vector128<short> rB, Vector128<short> rA, ref byte db, int o)
    {
        Vector128<byte> rg = Sse2.UnpackLow(Sse2.PackUnsignedSaturate(rR, rR), Sse2.PackUnsignedSaturate(rG, rG));
        Vector128<byte> ba = Sse2.UnpackLow(Sse2.PackUnsignedSaturate(rB, rB), Sse2.PackUnsignedSaturate(rA, rA));
        Sse2.UnpackLow(rg.AsUInt16(), ba.AsUInt16()).AsByte().StoreUnsafe(ref Unsafe.Add(ref db, o * 4));
        Sse2.UnpackHigh(rg.AsUInt16(), ba.AsUInt16()).AsByte().StoreUnsafe(ref Unsafe.Add(ref db, (o + 4) * 4));
    }
    ...
    private static void LerpSingleSubsetArm(
        int loR, int loG, int loB, int loA, int hiR, int hiG, int hiB, int hiA,
        scoped ReadOnlySpan<int> wt, scoped ReadOnlySpan<int> sel, scoped Span<ColorRgba> dec)
    {
        ref int se = ref MemoryMarshal.GetReference(sel);
        ...
        for (int o = 0; o < 16; o += 8)
        {
            Vector64<byte> wv = Vector64.Create(
                (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o)),     (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 1)),
                (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 2)), (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 3)),
                (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 4)), (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 5)),
                (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 6)), (byte)Unsafe.Add(ref wtab, Unsafe.Add(ref se, o + 7)));
            Vector64<byte> invv = AdvSimd.Subtract(c64, wv);
            Vector128<ushort> rR = AdvSimd.ShiftRightLogical(AdvSimd.Add(AdvSimd.MultiplyWideningLowerAndAdd(AdvSimd.MultiplyWideningLower(loRv, invv), hiRv, wv), c32), 6);
            Vector128<ushort> rG = AdvSimd.ShiftRightLogical(AdvSimd.Add(AdvSimd.MultiplyWideningLowerAndAdd(AdvSimd.MultiplyWideningLower(loGv, invv), hiGv, wv), c32), 6);
            Vector128<ushort> rB = AdvSimd.ShiftRightLogical(AdvSimd.Add(AdvSimd.MultiplyWideningLowerAndAdd(AdvSimd.MultiplyWideningLower(loBv, invv), hiBv, wv), c32), 6);
            Vector128<ushort> rA = AdvSimd.ShiftRightLogical(AdvSimd.Add(AdvSimd.MultiplyWideningLowerAndAdd(AdvSimd.MultiplyWideningLower(loAv, invv), hiAv, wv), c32), 6);
            Vector64<byte> pR = AdvSimd.ExtractNarrowingLower(rR), pG = AdvSimd.ExtractNarrowingLower(rG);
            Vector64<byte> pB = AdvSimd.ExtractNarrowingLower(rB), pA = AdvSimd.ExtractNarrowingLower(rA);
            WeaveStoreArm(pR, pG, pB, pA, ref db, o);
        }
    ...
    private static void WeaveStoreArm(Vector64<byte> pR, Vector64<byte> pG, Vector64<byte> pB, Vector64<byte> pA, ref byte db, int o)
    {
        ...
        AdvSimd.Arm64.ZipLow(rgLo.AsUInt16(),  baLo.AsUInt16()).AsByte().StoreUnsafe(ref Unsafe.Add(ref db, o * 4));
        AdvSimd.Arm64.ZipHigh(rgLo.AsUInt16(), baLo.AsUInt16()).AsByte().StoreUnsafe(ref Unsafe.Add(ref db, (o + 2) * 4));
        AdvSimd.Arm64.ZipLow(rgHi.AsUInt16(),  baHi.AsUInt16()).AsByte().StoreUnsafe(ref Unsafe.Add(ref db, (o + 4) * 4));
        AdvSimd.Arm64.ZipHigh(rgHi.AsUInt16(), baHi.AsUInt16()).AsByte().StoreUnsafe(ref Unsafe.Add(ref db, (o + 6) * 4));
    }
    ...
    internal static int SelectPartition2(scoped ReadOnlySpan<ColorRgba> px,
        int meanR, int meanG, int meanB, float axR, float axG, float axB)
    {
        int desired = 0;
        ref ColorRgba p0 = ref MemoryMarshal.GetReference(px);
        for (int i = 0; i < 16; i++)
        {
            ref readonly ColorRgba q = ref Unsafe.Add(ref p0, i);
            float r = q.R - meanR, g = q.G - meanG, b = q.B - meanB;
            int subset = (r * axR + g * axG + b * axB) > 0f ? 1 : 0;
            desired |= subset << i;
        }
        ref ushort masks = ref MemoryMarshal.GetArrayDataReference(Bc7Tables.Part2Bitmasks);
        if (Sse41.IsSupported) return HammingMinX86(ref masks, desired);
        if (AdvSimd.Arm64.IsSupported) return HammingMinArm(ref masks, desired);
        int bestDiff = int.MaxValue, bestPat = 0;
        for (int p = 0; p < 64; p++)
        {
            int diff = BitOperations.PopCount((uint)((Unsafe.Add(ref masks, p) ^ desired) & 0xFFFF));
            diff = Math.Min(diff, 16 - diff);
            if (diff < bestDiff) { bestDiff = diff; bestPat = p; }
        }
        return bestPat;
    }

    private static int HammingMinX86(ref ushort masks, int desired)
    {
        Vector128<byte> lut = Vector128.Create((byte)0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4); // popcount(0..15)
        Vector128<byte> lowMask = Vector128.Create((byte)0x0F);
        Vector128<sbyte> ones = Vector128.Create((sbyte)1);
        Vector128<ushort> desv = Vector128.Create((ushort)desired);
        Vector128<ushort> c16 = Vector128.Create((ushort)16);
        Vector128<ushort> ramp = Vector128.Create((ushort)0, 1, 2, 3, 4, 5, 6, 7);
        Vector128<ushort> minKey = Vector128.Create((ushort)0xFFFF);
        for (int c = 0; c < 64; c += 8)
        {
            Vector128<byte> x = (Vector128.LoadUnsafe(ref Unsafe.Add(ref masks, c)) ^ desv).AsByte();
            Vector128<byte> lo = Ssse3.Shuffle(lut, x & lowMask);
            Vector128<byte> hi = Ssse3.Shuffle(lut, Sse2.ShiftRightLogical(x.AsUInt16(), 4).AsByte() & lowMask);
            Vector128<ushort> d = Ssse3.MultiplyAddAdjacent(lo + hi, ones).AsUInt16();  // popcount per 16-bit lane (0..16)
            Vector128<ushort> dmin = Sse41.Min(d, c16 - d);
            Vector128<ushort> key = Vector128.ShiftLeft(dmin, 8) | (ramp + Vector128.Create((ushort)c));
            minKey = Sse41.Min(minKey, key);
        }
        return Sse41.MinHorizontal(minKey).ToScalar() & 0xFF;       // PHMINPOSUW: low lane = min key
    }

    private static int HammingMinArm(ref ushort masks, int desired)
    {
        Vector128<ushort> desv = Vector128.Create((ushort)desired);
        Vector128<ushort> c16 = Vector128.Create((ushort)16);
        Vector128<ushort> ramp = Vector128.Create((ushort)0, 1, 2, 3, 4, 5, 6, 7);
        ...
        {
            Vector128<byte> x = (Vector128.LoadUnsafe(ref Unsafe.Add(ref masks, c)) ^ desv).AsByte();
            Vector128<ushort> d = AdvSimd.AddPairwiseWidening(AdvSimd.PopCount(x));   // vcnt + vpaddl.u8 -> popcount per 16-bit lane
            Vector128<ushort> dmin = AdvSimd.Min(d, AdvSimd.Subtract(c16, d));
            Vector128<ushort> key = AdvSimd.Or(Vector128.ShiftLeft(dmin, 8), AdvSimd.Add(ramp, Vector128.Create((ushort)c)));
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int To6(int c8, uint pbit)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int From6(int v, uint p)
    {
        uint t = ((uint)v << 1) | p;   // 6-bit + pbit -> 7-bit
        t = (t << 1) | (t >> 6);       // -> 8-bit
        return (int)t;
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int From5(int v) { uint t = (uint)v; t = (t << 3) | (t >> 2); return (int)t; }

    // 5-bit endpoint quantization with a p-bit — mode 7 (5-bit value + p-bit = 6-bit precision).
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int To5(int c8, uint pbit)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int From5(int v, uint p)
    {
        uint t = ((uint)v << 1) | p;   // 5-bit + pbit -> 6-bit
        t = (t << 2) | (t >> 4);       // -> 8-bit
        return (int)t;
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static float ClampF(float v) => v < 0f ? 0f : (v > 255f ? 255f : v);
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int RoundPos(float c) => (int)(c + 0.5f);

    // Single subset, RGB (all 16 pixels). Used by mode 6.
    private static bool ComputeLs3D(scoped ReadOnlySpan<int> w, scoped ReadOnlySpan<ColorRgba> px, scoped ReadOnlySpan<float> tab,
        float tr, float tg, float tb, scoped Span<float> xl, scoped Span<float> xh)
    {
        ...
        xh[0] = ClampF(iz00 * q0r + iz01 * q1r); xl[0] = ClampF(iz10 * q0r + iz11 * q1r);
        xh[1] = ClampF(iz00 * q0g + iz01 * q1g); xl[1] = ClampF(iz10 * q0g + iz11 * q1g);
        xh[2] = ClampF(iz00 * q0b + iz01 * q1b); xl[2] = ClampF(iz10 * q0b + iz11 * q1b);
        return true;
    }

    // Single subset, RGBA (all 16 pixels). Used by mode 6 RGBA.
    private static bool ComputeLs4D(scoped ReadOnlySpan<int> w, scoped ReadOnlySpan<ColorRgba> px, scoped ReadOnlySpan<float> tab,
        float tr, float tg, float tb, float ta, scoped Span<float> xl, scoped Span<float> xh)
    {
        ...
        xh[1] = ClampF(iz00 * q0g + iz01 * q1g); xl[1] = ClampF(iz10 * q0g + iz11 * q1g);
        xh[2] = ClampF(iz00 * q0b + iz01 * q1b); xl[2] = ClampF(iz10 * q0b + iz11 * q1b);
        xh[3] = ClampF(iz00 * q0a + iz01 * q1a); xl[3] = ClampF(iz10 * q0a + iz11 * q1a);
        return true;
    }

    // Single channel (all 16 pixels). Used by modes 4/5 for the scalar (rotated) plane.
    private static bool ComputeLs1D(scoped ReadOnlySpan<int> w, scoped ReadOnlySpan<ColorRgba> px, int comp, scoped ReadOnlySpan<float> tab,
        float t, out float xl, out float xh)
    {
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            ref readonly ColorRgba q = ref Unsafe.Add(ref pr, i);
            byte c = comp == 0 ? q.R : comp == 1 ? q.G : comp == 2 ? q.B : q.A;
            q0 += Unsafe.Add(ref tabr, o + 3) * c;
        }
        float det = z00 * z11 - z10 * z10;
        if (MathF.Abs(det) < 1e-8f) { xl = xh = 0; return false; }
        det = 1f / det;
        float iz00 = z11 * det, iz01 = -z10 * det, iz10 = -z10 * det, iz11 = z00 * det;
        float q1 = t - q0;
    ...
    private static bool ComputeLs3DSub(scoped ReadOnlySpan<int> w, scoped ReadOnlySpan<ColorRgba> px, int subsetMask, int subset,
        scoped ReadOnlySpan<float> tab, float tr, float tg, float tb, scoped Span<float> xl, scoped Span<float> xh)
    {
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            int o = Unsafe.Add(ref wr, i) * 4; z00 += Unsafe.Add(ref tabr, o); z10 += Unsafe.Add(ref tabr, o + 1); z11 += Unsafe.Add(ref tabr, o + 2);
            ref readonly ColorRgba q = ref Unsafe.Add(ref pr, i);
            float ww = Unsafe.Add(ref tabr, o + 3); q0r += ww * q.R; q0g += ww * q.G; q0b += ww * q.B;
        }
        float det = z00 * z11 - z10 * z10;
        if (MathF.Abs(det) < 1e-8f) return false;
        det = 1f / det;
    ...
    private static void DetermineUniquePbits(int totalComps, int compBits, scoped ReadOnlySpan<float> xl, scoped ReadOnlySpan<float> xh,
        scoped Span<int> minC, scoped Span<int> maxC, scoped Span<uint> pbits)
    {
        int totalBits = compBits + 1, iscalep = (1 << totalBits) - 1; float scalep = iscalep;
        float bestErr0 = 1e9f, bestErr1 = 1e9f;
        Span<int> xmin = stackalloc int[4]; Span<int> xmax = stackalloc int[4];
        for (int p = 0; p < 2; p++)
        {
            for (int c = 0; c < 4; c++)
            {
            ...
            {
                int sl = xmin[i] << (8 - totalBits); sl |= sl >> totalBits;
                int sh = xmax[i] << (8 - totalBits); sh |= sh >> totalBits;
                float d0 = sl - xl[i] * 255f, d1 = sh - xh[i] * 255f;
                err0 += d0 * d0; err1 += d1 * d1;
            }
            if (err0 < bestErr0) { bestErr0 = err0; pbits[0] = (uint)p; for (int j = 0; j < 4; j++) minC[j] = xmin[j] >> 1; }
            if (err1 < bestErr1) { bestErr1 = err1; pbits[1] = (uint)p; for (int j = 0; j < 4; j++) maxC[j] = xmax[j] >> 1; }
        }
    }

    private static void DetermineSharedPbits(int totalComps, int compBits, scoped ReadOnlySpan<float> xl, scoped ReadOnlySpan<float> xh,
        scoped Span<int> minC, scoped Span<int> maxC, scoped Span<uint> pbits)
    {
        int totalBits = compBits + 1, iscalep = (1 << totalBits) - 1; float scalep = iscalep;
        float bestErr = 1e9f;
        ...
        for (int p = 0; p < 2; p++)
        {
            ...
            if (err < bestErr) { bestErr = err; pbits[0] = (uint)p; pbits[1] = (uint)p; for (int j = 0; j < 4; j++) { minC[j] = xmin[j] >> 1; maxC[j] = xmax[j] >> 1; } }
        }
    }

    // Per-subset RGB keyed on a 3-subset partition (Partition3[pat3*16+i] == subset). Used by modes 0/2.
    private static bool ComputeLs3DSubP(scoped ReadOnlySpan<int> w, scoped ReadOnlySpan<ColorRgba> px, int pat3, int subset,
        scoped ReadOnlySpan<float> tab, float tr, float tg, float tb, scoped Span<float> xl, scoped Span<float> xh)
    {
        float z00 = 0, z10 = 0, z11 = 0, q0r = 0, q0g = 0, q0b = 0;
        ref int wr = ref MemoryMarshal.GetReference(w); ref ColorRgba pr = ref MemoryMarshal.GetReference(px); ref float tabr = ref MemoryMarshal.GetReference(tab); ref byte p3r = ref MemoryMarshal.GetArrayDataReference(Bc7Tables.Partition3);
        for (int i = 0; i < 16; i++)
        {
    ...
    private static bool ComputeLs4DSub(scoped ReadOnlySpan<int> w, scoped ReadOnlySpan<ColorRgba> px, int patBits, int subset,
        scoped ReadOnlySpan<float> tab, float tr, float tg, float tb, float ta, scoped Span<float> xl, scoped Span<float> xh)
    {
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            int o = Unsafe.Add(ref wr, i) * 4; z00 += Unsafe.Add(ref tabr, o); z10 += Unsafe.Add(ref tabr, o + 1); z11 += Unsafe.Add(ref tabr, o + 2);
            ref readonly ColorRgba q = ref Unsafe.Add(ref pr, i);
            float ww = Unsafe.Add(ref tabr, o + 3); q0r += ww * q.R; q0g += ww * q.G; q0b += ww * q.B; q0a += ww * q.A;
        }
        float det = z00 * z11 - z10 * z10;
        if (MathF.Abs(det) < 1e-8f) return false;
        det = 1f / det;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int To4(int c8, uint pbit)
    {
        int q5 = (c8 * 31 + 127) / 255;
        if ((q5 & 1) != (int)pbit)
        {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int From4(int v, uint p)
    {
        uint t = ((uint)v << 1) | p;   // 4-bit + pbit -> 5-bit
        t = (t << 3) | (t >> 2);       // -> 8-bit
        return (int)t;
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int To7(int c8) => (c8 * 127 + 127) / 255;
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int To6(int c8) => (c8 * 63 + 127) / 255;
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int From7(int v) { uint t = (uint)v; return (int)((t << 1) | (t >> 6)); }
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int From6(int v) { uint t = (uint)v; return (int)((t << 2) | (t >> 4)); }
    ...
    private static void PackMode5Solid(scoped Span<byte> block, ColorRgba c)
    {
        Bc7Tables.EndpointPair er = Bc7Tables.Mode5OptimalEndpoints[c.R];
        Bc7Tables.EndpointPair eg = Bc7Tables.Mode5OptimalEndpoints[c.G];
        Bc7Tables.EndpointPair eb = Bc7Tables.Mode5OptimalEndpoints[c.B];
        ...
        block[9] = 0xaa; block[10] = 0xaa; block[11] = 0xaa;
    ...
    private const float UniquePbitDiscount = 0.85f;
    private const float SharedPbitDiscount = 0.95f;
    ...
    /// </summary>
    private static float AnalyticalQuantEstSse(int eLevels, int wLevels, int numChans,
        scoped ReadOnlySpan<int> spans, scoped ReadOnlySpan<float> spanWeights, float endpointWeightScale, int numPixels)
    {
        float dep = 1.0f / (eLevels - 1); // endpoint quant step
        float dw  = 1.0f / (wLevels - 1); // weight quant step
    ...
    private static float AnalyticalQuantEstSse(int eLevels, int wLevels, int span, float spanWeight,
        float endpointWeightScale, int numPixels)
    {
        float dep = 1.0f / (eLevels - 1);
        float dw  = 1.0f / (wLevels - 1);
        float n = wLevels;
        float abSum = (2.0f * n - 1.0f) / (3.0f * (n - 1.0f));
        float pixelSse = (eLevels == 256)
            ? 0.0f
