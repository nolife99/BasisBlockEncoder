...
{
    static bool PackMode1Or3Rgb(scoped Span<byte> block,
        ...
        out uint trueSse)
    {
        ...
        var bestPat = SelectPartition2(px, meanR, meanG, meanB, axR, axG, axB);
        int patBits = Bc7Tables.Part2Bitmasks[bestPat];

        // (b) per-subset totals / means
        Span<int> totR = stackalloc int[2];
        Span<int> totG = stackalloc int[2];
        Span<int> totB = stackalloc int[2];
        Span<int> totC = stackalloc int[2];
        ...
        Span<int> meanRs = stackalloc int[2];
        Span<int> meanGs = stackalloc int[2];
        Span<int> meanBs = stackalloc int[2];
        for (var s = 0; s < 2; s++)
        {
            int t = totC[s], h = t >> 1;
            meanRs[s] = (totR[s] + h) / t;
        ...
        Span<int> ar = stackalloc int[2];
        Span<int> ag = stackalloc int[2];
        Span<int> ab = stackalloc int[2];
        ...
        Span<int> lowDot = stackalloc int[2];
        lowDot[0] = lowDot[1] = int.MaxValue;
        Span<int> highDot = stackalloc int[2];
        highDot[0] = highDot[1] = int.MinValue;
        ref var _pp3 = ref MemoryMarshal.GetReference(px);
        for (var i = 0; i < 16; i++)
        {
            ref readonly var _qq3 = ref Unsafe.Add(ref _pp3, i);
            var s = patBits >> i & 1;
            var dot = _qq3.R * ar[s] + _qq3.G * ag[s] + _qq3.B * ab[s] + i;
            if (dot < lowDot[s]) lowDot[s] = dot;
            if (dot > highDot[s]) highDot[s] = dot;
        }

        Span<int> loC = stackalloc int[2];
        Span<int> hiC = stackalloc int[2];
        for (var s = 0; s < 2; s++)
        {
            loC[s] = lowDot[s] & 0xF;
            hiC[s] = highDot[s] & 0xF;
        ...
        var disc1 = (flags & Bc7Flags.PBitOpt) != 0 ? UniquePbitDiscount : 1f; // mode 1
        var disc3 = (flags & Bc7Flags.PBitOpt) != 0 ? SharedPbitDiscount : 1f; // mode 3
        Span<int> spans = stackalloc int[4];
        float q1 = 0, q3 = 0;
        for (var s = 0; s < 2; s++)
        {
            spans[0] = px[hiC[s]].R - px[loC[s]].R;
            spans[1] = px[hiC[s]].G - px[loC[s]].G;
            spans[2] = px[hiC[s]].B - px[loC[s]].B;
        ...
        Span<uint> lg = stackalloc uint[2];
        Span<uint> lb = stackalloc uint[2];
        Span<uint> hr = stackalloc uint[2];
        Span<uint> hg = stackalloc uint[2];
        Span<uint> hb = stackalloc uint[2];
        ...
        Span<int> eLoR = stackalloc int[2];
        Span<int> eLoG = stackalloc int[2];
        Span<int> eLoB = stackalloc int[2];
        Span<int> eHiR = stackalloc int[2];
        Span<int> eHiG = stackalloc int[2];
        Span<int> eHiB = stackalloc int[2];
        ...
        if (mode1Est < mode3Est)
        {
            ...
            for (var s = 0; s < 2; s++)
            {
                ...
                {
                    var p = (uint)(Math.Max(px[lc].R + px[lc].G + px[lc].B, px[hc].R + px[hc].G + px[hc].B) >= 129 * 3 ? 1 : 0);
                    pb[s] = p;
                    lr[s] = (uint)To6(px[lc].R, p);
                    lg[s] = (uint)To6(px[lc].G, p);
                    lb[s] = (uint)To6(px[lc].B, p);
                    hr[s] = (uint)To6(px[hc].R, p);
                    hg[s] = (uint)To6(px[hc].G, p);
                    hb[s] = (uint)To6(px[hc].B, p);
                }
            }
            ...
            for (var s = 0; s < 2; s++)
            {
                ...
                else
                {
                    ...
                    var p = (uint)(MathF.Max(l, h) >= 129f * 3f ? 1 : 0);
                    pb[s] = p;
                    lr[s] = (uint)To6(RoundPos(xl[0]), p);
                    lg[s] = (uint)To6(RoundPos(xl[1]), p);
                    lb[s] = (uint)To6(RoundPos(xl[2]), p);
                    hr[s] = (uint)To6(RoundPos(xh[0]), p);
                    hg[s] = (uint)To6(RoundPos(xh[1]), p);
                    hb[s] = (uint)To6(RoundPos(xh[2]), p);
                }
            }

            for (var s = 0; s < 2; s++)
            {
                eLoR[s] = From6((int)lr[s], pb[s]);
                eLoG[s] = From6((int)lg[s], pb[s]);
                eLoB[s] = From6((int)lb[s], pb[s]);
                eHiR[s] = From6((int)hr[s], pb[s]);
                eHiG[s] = From6((int)hg[s], pb[s]);
                eHiB[s] = From6((int)hb[s], pb[s]);
            }
            ...
            else
            {
                ...
                if (wantSse)
                {
                    var sx = SseMultiSubsetRgb(eLoR, eLoG, eLoB, eHiR, eHiG, eHiB, Bc7Tables.Partition2, bestPat * 16, Bc7Tables.Weights3, w, px);
        ...
        else
        {
            ...
            for (var s = 0; s < 2; s++)
            {
                ...
                else
                {
                    ...
                    pbits[s * 2] = lp;
                    lr[s] = (uint)To7(px[lc].R, lp);
                    lg[s] = (uint)To7(px[lc].G, lp);
                    lb[s] = (uint)To7(px[lc].B, lp);
                    var hp = (uint)(px[hc].R + px[hc].G + px[hc].B >= 129 ? 1 : 0);
                    pbits[s * 2 + 1] = hp;
                    hr[s] = (uint)To7(px[hc].R, hp);
                    hg[s] = (uint)To7(px[hc].G, hp);
                    hb[s] = (uint)To7(px[hc].B, hp);
                }
            }
            ...
            for (var s = 0; s < 2; s++)
            {
                ...
                else
                {
                    ...
                    var lp = (uint)(lr8 + lg8 + lb8 >= 129 ? 1 : 0);
                    pbits[s * 2] = lp;
                    lr[s] = (uint)To7(lr8, lp);
                    lg[s] = (uint)To7(lg8, lp);
                    lb[s] = (uint)To7(lb8, lp);
                    var hp = (uint)(hr8 + hg8 + hb8 >= 129 ? 1 : 0);
                    pbits[s * 2 + 1] = hp;
                    hr[s] = (uint)To7(hr8, hp);
                    hg[s] = (uint)To7(hg8, hp);
                    hb[s] = (uint)To7(hb8, hp);
                }
            }

            for (var s = 0; s < 2; s++)
            {
                eLoR[s] = From7((int)lr[s], pbits[s * 2]);
                eLoG[s] = From7((int)lg[s], pbits[s * 2]);
                eLoB[s] = From7((int)lb[s], pbits[s * 2]);
                eHiR[s] = From7((int)hr[s], pbits[s * 2 + 1]);
                eHiG[s] = From7((int)hg[s], pbits[s * 2 + 1]);
                eHiB[s] = From7((int)hb[s], pbits[s * 2 + 1]);
            }
            ...
            else
            {
                ...
                if (wantSse)
                {
                    var sx = SseMultiSubsetRgb(eLoR, eLoG, eLoB, eHiR, eHiG, eHiB, Bc7Tables.Partition2, bestPat * 16, Bc7Tables.Weights2, w, px);
                    trueSse = sx > uint.MaxValue ? uint.MaxValue : (uint)sx;
                }
            }
    ...
    static void EvalWeightsMode3(scoped ReadOnlySpan<ColorRgba> px,
        scoped Span<int> w,
        ...
        int subsetBitmask)
    {
        Span<int> dr = stackalloc int[2];
        Span<int> dg = stackalloc int[2];
        Span<int> db = stackalloc int[2];
        Span<float> f = stackalloc float[2];
        Span<int> sofs = stackalloc int[2];
        Span<int> lr = stackalloc int[2];
        Span<int> lg = stackalloc int[2];
        Span<int> lb = stackalloc int[2];
        for (var s = 0; s < 2; s++)
        {
            lr[s] = From7((int)blr[s], pbits[s * 2]);
            lg[s] = From7((int)blg[s], pbits[s * 2]);
            lb[s] = From7((int)blb[s], pbits[s * 2]);
    ...
    static void EncodeMode3(scoped Span<byte> block,
        uint partId,
        scoped Span<uint> lr,
        scoped Span<uint> lg,
        scoped Span<uint> lb,
        ...
        scoped ReadOnlySpan<int> weights)
    {
        ...
        var y = (ulong)hg[1] >> 5 | (ulong)lb[0] << 2 | (ulong)hb[0] << 9
    ...
    static void EvalWeightsMode1(scoped ReadOnlySpan<ColorRgba> px,
        scoped Span<int> w,
        scoped ReadOnlySpan<uint> blr,
        scoped ReadOnlySpan<uint> blg,
        scoped ReadOnlySpan<uint> blb,
        ...
        int subsetBitmask)
    {
        ...
        for (var s = 0; s < 2; s++)
        {
            lr[s] = From6((int)blr[s], pbits[s]);
            lg[s] = From6((int)blg[s], pbits[s]);
            lb[s] = From6((int)blb[s], pbits[s]);
    ...
    static void EncodeMode1(scoped Span<byte> block,
        uint partId,
        scoped Span<uint> lr,
        scoped Span<uint> lg,
        scoped Span<uint> lb,
