...
{
    static bool PackMode7Rgba(scoped Span<byte> block,
        ...
        out uint trueSse)
    {
        ...
        for (var i = 0; i < 16; i++)
        {
            ...
            var s = r * axR + g * axG + b * axB + a * axA > 0f ? 1 : 0;
            desired |= s << i;
        }
        ...
        for (var p = 0; p < 64; p++)
        {
            var diff = BitOperations.PopCount((uint)((Bc7Tables.Part2Bitmasks[p] ^ desired) & 0xFFFF));
            var md = Math.Min(diff, 16 - diff);
            var key = md << 8 | p;
            if (key < best) best = key;
        }

        var bestPat = best & 0xFF;
        int patBits = Bc7Tables.Part2Bitmasks[bestPat];

        // (b) per-subset totals/means
        Span<int> tR = stackalloc int[2];
        Span<int> tG = stackalloc int[2];
        Span<int> tB = stackalloc int[2];
        Span<int> tA = stackalloc int[2];
        Span<int> tC = stackalloc int[2];
        ...
        Span<int> aa = stackalloc int[2];
        Span<float> cov = stackalloc float[10];
        float slam = 0;
        for (var s = 0; s < 2; s++)
        {
            var o = s * 10;
            var bmv = Math.Max(Math.Max(icov[o], icov[o + 4]), Math.Max(icov[o + 7], icov[o + 9]));
            for (var j = 0; j < 10; j++) cov[j] = icov[o + j];
            var sc = bmv != 0 ? 1f / bmv : 0f;
            float wx = sc * cov[0], wy = sc * cov[4], wz = sc * cov[7], wa = sc * cov[9];
            var x0 = cov[0] * wx + cov[1] * wy + cov[2] * wz + cov[3] * wa;
            var y0 = cov[1] * wx + cov[4] * wy + cov[5] * wz + cov[6] * wa;
            var z0 = cov[2] * wx + cov[5] * wy + cov[7] * wz + cov[8] * wa;
            var w0 = cov[3] * wx + cov[6] * wy + cov[8] * wz + cov[9] * wa;
            var x1 = cov[0] * x0 + cov[1] * y0 + cov[2] * z0 + cov[3] * w0;
            var y1 = cov[1] * x0 + cov[4] * y0 + cov[5] * z0 + cov[6] * w0;
            var z1 = cov[2] * x0 + cov[5] * y0 + cov[7] * z0 + cov[8] * w0;
            var w1 = cov[3] * x0 + cov[6] * y0 + cov[8] * z0 + cov[9] * w0;
            slam += EstimateSlamToLineSse4D(cov, x1, y1, z1, w1, out _);
            int sr = 256, sg = 256, sb = 256, sa = 256;
            var k = MathF.Max(MathF.Max(MathF.Abs(x1), MathF.Abs(y1)), MathF.Max(MathF.Abs(z1), MathF.Abs(w1)));
            if (k >= SmallFloat)
            {
                var m = 2048f / k;
                sr = (int)(x1 * m);
        ...
        for (var i = 0; i < 16; i++)
        {
            ...
            var s = patBits >> i & 1;
            var dot = _qq2.R * ar[s] + _qq2.G * ag[s] + _qq2.B * ab[s] + _qq2.A * aa[s] + i;
        ...
        var disc = (flags & Bc7Flags.PBitOpt) != 0 ? UniquePbitDiscount : 1f;
        ...
        for (var s = 0; s < 2; s++)
        {
            ...
            else
            {
                ...
                pbits[s * 2] = lp;
                pbits[s * 2 + 1] = hp;
                lr[s] = (uint)To5(px[lc].R, lp);
                lg[s] = (uint)To5(px[lc].G, lp);
                lb[s] = (uint)To5(px[lc].B, lp);
                la[s] = (uint)To5(px[lc].A, lp);
                hr[s] = (uint)To5(px[hc].R, hp);
                hg[s] = (uint)To5(px[hc].G, hp);
                hb[s] = (uint)To5(px[hc].B, hp);
                ha[s] = (uint)To5(px[hc].A, hp);
            }
        }
        ...
        for (var s = 0; s < 2; s++)
        {
            ...
            else
            {
                ...
                pbits[s * 2] = lp;
                pbits[s * 2 + 1] = hp;
                lr[s] = (uint)To5(RoundPos(xl[0]), lp);
                lg[s] = (uint)To5(RoundPos(xl[1]), lp);
                lb[s] = (uint)To5(RoundPos(xl[2]), lp);
                la[s] = (uint)To5(la8, lp);
                hr[s] = (uint)To5(RoundPos(xh[0]), hp);
                hg[s] = (uint)To5(RoundPos(xh[1]), hp);
                hb[s] = (uint)To5(RoundPos(xh[2]), hp);
                ha[s] = (uint)To5(ha8, hp);
            }
        }
        ...
        for (var s = 0; s < 2; s++)
        {
            eLoR[s] = From5((int)lr[s], pbits[s * 2]);
            eLoG[s] = From5((int)lg[s], pbits[s * 2]);
            eLoB[s] = From5((int)lb[s], pbits[s * 2]);
            eLoA[s] = From5((int)la[s], pbits[s * 2]);
            eHiR[s] = From5((int)hr[s], pbits[s * 2 + 1]);
            eHiG[s] = From5((int)hg[s], pbits[s * 2 + 1]);
            eHiB[s] = From5((int)hb[s], pbits[s * 2 + 1]);
            eHiA[s] = From5((int)ha[s], pbits[s * 2 + 1]);
        }

        if (wantSse)
        {
            var sx = SseMultiSubsetRgba(eLoR, eLoG, eLoB, eLoA, eHiR, eHiG, eHiB, eHiA, Bc7Tables.Partition2, bestPat * 16, Bc7Tables.Weights2, w, px);
        ...
        int subsetBitmask)
    {
        ...
        Span<int> la = stackalloc int[2];
        for (var s = 0; s < 2; s++)
        {
            uint lp = pbits[s * 2], hp = pbits[s * 2 + 1];
            lr[s] = From5((int)blr[s], lp);
            lg[s] = From5((int)blg[s], lp);
            lb[s] = From5((int)blb[s], lp);
            la[s] = From5((int)bla[s], lp);
    ...
    static void EncodeMode7(scoped Span<byte> block,
        uint partId,
        scoped Span<uint> lr,
        scoped Span<uint> lg,
        scoped Span<uint> lb,
