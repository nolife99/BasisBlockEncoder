...
public sealed class Bc4Encoder : IBlockEncoder, IBlockEncoderBatch
{
...
public sealed class Bc5Encoder : IBlockEncoder, IBlockEncoderBatch
{
    ...
    void IBlockEncoderBatch.EncodeBatch(scoped ReadOnlySpan<ColorRgba> px, scoped Span<byte> output)
    {
        Bc4Block.EncodeAlphaBatch(px, output, _channel0, 16, 0); // channel0 -> bytes 0..7
        Bc4Block.EncodeAlphaBatch(px, output, _channel1, 16, 8); // channel1 -> bytes 8..15
    }
}