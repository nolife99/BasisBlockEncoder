...
static class LdrDecode
{
    ...
    public static void DecodeBc4(scoped ReadOnlySpan<byte> block, scoped Span<byte> outVals)
    {
        ...
        var sel = block[2] | (ulong)block[3] << 8 | (ulong)block[4] << 16 |
            (ulong)block[5] << 24 | (ulong)block[6] << 32 | (ulong)block[7] << 40;

        for (var i = 0; i < 16; i++) outVals[i] = (byte)pal[(int)(sel >> 3 * i & 7)];
    }
    ...
    public static void DecodeBc1(scoped ReadOnlySpan<byte> block, scoped Span<ColorRgba> outPx)
    {
        ...
        r[0] = Scale5To8(c0 >> 11 & 31);
        g[0] = Scale6To8(c0 >> 5 & 63);
        b[0] = Scale5To8(c0 & 31);
        r[1] = Scale5To8(c1 >> 11 & 31);
        g[1] = Scale6To8(c1 >> 5 & 63);
        b[1] = Scale5To8(c1 & 31);
        if (c0 > c1)
        {
            r[2] = (2 * r[0] + r[1]) / 3;
        ...
        var sel = block[4] | (uint)block[5] << 8 | (uint)block[6] << 16 | (uint)block[7] << 24;
        for (var i = 0; i < 16; i++)
        {
            var sIdx = (int)(sel >> 2 * i & 3);
            outPx[i] = new()
            {
                R = (byte)r[sIdx],
                G = (byte)g[sIdx],
    ...
    public static void DecodeBc3(scoped ReadOnlySpan<byte> block, scoped Span<ColorRgba> outPx)
    {
        ...
        var rgba = MemoryMarshal.AsBytes(outPx);
        for (var i = 0; i < 16; i++) rgba[i * 4 + 3] = alpha[i];
    }
