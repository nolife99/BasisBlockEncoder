...
internal static partial class Bc4Block
{
    ...
    private static readonly byte[] BaseTran = { 1, 7, 6, 5, 4, 3, 2, 0 };

    // Pre-shifted tables for the scalar path (from the reference encoder).
    private static readonly uint[] STran0 = { 1u, 7u, 6u, 5u, 4u, 3u, 2u, 0u };
    private static readonly uint[] STran1 = { 1u << 3, 7u << 3, 6u << 3, 5u << 3, 4u << 3, 3u << 3, 2u << 3, 0u << 3 };
    private static readonly uint[] STran2 = { 1u << 6, 7u << 6, 6u << 6, 5u << 6, 4u << 6, 3u << 6, 2u << 6, 0u << 6 };
    private static readonly uint[] STran3 = { 1u << 9, 7u << 9, 6u << 9, 5u << 9, 4u << 9, 3u << 9, 2u << 9, 0u << 9 };
    ...
    internal static void EncodeScalar(scoped ReadOnlySpan<ColorRgba> pixels, scoped Span<byte> output, int channel)
    {
        ...
        switch (channel)
        {
            case 0: for (int i = 0; i < 16; i++) p[i] = pixels[i].R; break;
            case 1: for (int i = 0; i < 16; i++) p[i] = pixels[i].G; break;
            case 2: for (int i = 0; i < 16; i++) p[i] = pixels[i].B; break;
            default: for (int i = 0; i < 16; i++) p[i] = pixels[i].A; break;
        }
    ...
    internal static void EncodeChannel(scoped ReadOnlySpan<int> p, scoped Span<byte> output)
    {
        ...
        for (int i = 1; i < 16; i++) { int v = p[i]; if (v < minV) minV = v; if (v > maxV) maxV = v; }

        output[0] = (byte)maxV;
        output[1] = (byte)minV;
        if (maxV == minV) { output.Slice(2, 6).Clear(); return; }

        int delta = maxV - minV;
        int t0 = delta * 13, t1 = delta * 11, t2 = delta * 9, t3 = delta * 7, t4 = delta * 5, t5 = delta * 3, t6 = delta * 1;
        int bias = 4 - minV * 14;

        ref uint s0 = ref MemoryMarshal.GetArrayDataReference(STran0);
        ref uint s1 = ref MemoryMarshal.GetArrayDataReference(STran1);
        ref uint s2 = ref MemoryMarshal.GetArrayDataReference(STran2);
        ref uint s3 = ref MemoryMarshal.GetArrayDataReference(STran3);
        ref int pr = ref MemoryMarshal.GetReference(p);
    ...
    private static int Cross(int v, int t0, int t1, int t2, int t3, int t4, int t5, int t6)
    ...
    internal static void EncodeV256(scoped ReadOnlySpan<ColorRgba> px, scoped Span<byte> output, int channel)
    {
        ...
        Vector256<int> c0 = (Vector256.ShiftRightLogical(Vector256.LoadUnsafe(ref pru, 0), (byte)sh) & mask).AsInt32();
        Vector256<int> c1 = (Vector256.ShiftRightLogical(Vector256.LoadUnsafe(ref pru, 8), (byte)sh) & mask).AsInt32();

        int minV = HMin(Vector256.Min(c0, c1)), maxV = HMax(Vector256.Max(c0, c1));
        output[0] = (byte)maxV; output[1] = (byte)minV;
        if (maxV == minV) { output.Slice(2, 6).Clear(); return; }
        ...
        ref int cr = ref MemoryMarshal.GetReference(cnt);
        n0.StoreUnsafe(ref cr, 0); n1.StoreUnsafe(ref cr, 8);
        PackFromCounts(output, cnt);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static Vector256<int> CountGE(Vector256<int> v, Vector256<int> t0, Vector256<int> t1, Vector256<int> t2,
        Vector256<int> t3, Vector256<int> t4, Vector256<int> t5, Vector256<int> t6)
    {
        ...
        c -= Vector256.GreaterThanOrEqual(v, t0); c -= Vector256.GreaterThanOrEqual(v, t1);
        c -= Vector256.GreaterThanOrEqual(v, t2); c -= Vector256.GreaterThanOrEqual(v, t3);
        c -= Vector256.GreaterThanOrEqual(v, t4); c -= Vector256.GreaterThanOrEqual(v, t5);
    ...
    internal static void EncodeV128(scoped ReadOnlySpan<ColorRgba> px, scoped Span<byte> output, int channel)
    {
        ...
        Vector128<int> g0 = (Vector128.ShiftRightLogical(Vector128.LoadUnsafe(ref pru, 0), (byte)sh) & mask).AsInt32();
        Vector128<int> g1 = (Vector128.ShiftRightLogical(Vector128.LoadUnsafe(ref pru, 4), (byte)sh) & mask).AsInt32();
        Vector128<int> g2 = (Vector128.ShiftRightLogical(Vector128.LoadUnsafe(ref pru, 8), (byte)sh) & mask).AsInt32();
        Vector128<int> g3 = (Vector128.ShiftRightLogical(Vector128.LoadUnsafe(ref pru, 12), (byte)sh) & mask).AsInt32();

        int minV = HMin(Vector128.Min(Vector128.Min(g0, g1), Vector128.Min(g2, g3)));
        int maxV = HMax(Vector128.Max(Vector128.Max(g0, g1), Vector128.Max(g2, g3)));
        output[0] = (byte)maxV; output[1] = (byte)minV;
        if (maxV == minV) { output.Slice(2, 6).Clear(); return; }
        ...
        Span<int> cnt = stackalloc int[16];
        ref int cr = ref MemoryMarshal.GetReference(cnt);
        CountGE(g0 * f14 + bv, t0, t1, t2, t3, t4, t5, t6).StoreUnsafe(ref cr, 0);
        CountGE(g1 * f14 + bv, t0, t1, t2, t3, t4, t5, t6).StoreUnsafe(ref cr, 4);
        CountGE(g2 * f14 + bv, t0, t1, t2, t3, t4, t5, t6).StoreUnsafe(ref cr, 8);
        CountGE(g3 * f14 + bv, t0, t1, t2, t3, t4, t5, t6).StoreUnsafe(ref cr, 12);
        PackFromCounts(output, cnt);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static Vector128<int> CountGE(Vector128<int> v, Vector128<int> t0, Vector128<int> t1, Vector128<int> t2,
        Vector128<int> t3, Vector128<int> t4, Vector128<int> t5, Vector128<int> t6)
    {
        Vector128<int> c = Vector128<int>.Zero;
        c -= Vector128.GreaterThanOrEqual(v, t0); c -= Vector128.GreaterThanOrEqual(v, t1);
        c -= Vector128.GreaterThanOrEqual(v, t2); c -= Vector128.GreaterThanOrEqual(v, t3);
        c -= Vector128.GreaterThanOrEqual(v, t4); c -= Vector128.GreaterThanOrEqual(v, t5);
    ...
    // ---------------- shared tail ----------------
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void PackFromCounts(scoped Span<byte> output, scoped ReadOnlySpan<int> cnt)
    {
        ref byte bt = ref MemoryMarshal.GetArrayDataReference(BaseTran);
        ref int cr = ref MemoryMarshal.GetReference(cnt);
        ulong f = 0;
        for (int i = 0; i < 16; i++) f |= (ulong)Unsafe.Add(ref bt, Unsafe.Add(ref cr, i)) << (3 * i);
        WritePacked(output, f);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void WritePacked(scoped Span<byte> output, ulong f)
    {
        output[2] = (byte)f; output[3] = (byte)(f >> 8); output[4] = (byte)(f >> 16);
        output[5] = (byte)(f >> 24); output[6] = (byte)(f >> 32); output[7] = (byte)(f >> 40);
    }

    // horizontal min/max (same shape as the BC7 HMin/HMax helpers)
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int HMin(Vector128<int> m) { m = Vector128.Min(m, Vector128.Shuffle(m, Vector128.Create(2, 3, 0, 1))); m = Vector128.Min(m, Vector128.Shuffle(m, Vector128.Create(1, 0, 3, 2))); return m.ToScalar(); }
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int HMax(Vector128<int> m) { m = Vector128.Max(m, Vector128.Shuffle(m, Vector128.Create(2, 3, 0, 1))); m = Vector128.Max(m, Vector128.Shuffle(m, Vector128.Create(1, 0, 3, 2))); return m.ToScalar(); }
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
