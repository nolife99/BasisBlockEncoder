...
static partial class Bc1Block
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static int Scale6To8(int v) => v << 2 | v >> 4;

    static int To5(int v)
    {
        v = v * 31 + 128;
        return v + (v >> 8) >> 8;
    }

    static int To6(int v)
    {
        v = v * 63 + 128;
        return v + (v >> 8) >> 8;
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static uint Pack565(int r, int g, int b) => (uint)(r << 11 | g << 5 | b);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static int Clamp5(int v) => (uint)v > 31u ? ~v >> 31 & 31 : v;
    ...
    static void PrepareSingleColorTable(byte[] hiT, byte[] loT, int size)
    {
        ...
        {
            var lowestE = 256;
            for (var lo = 0; lo < size; lo++)
            {
                var loE = size == 32 ? Scale5To8(lo) : Scale6To8(lo);
                for (var hi = 0; hi < size; hi++)
                {
                    var hiE = size == 32 ? Scale5To8(hi) : Scale6To8(hi);
                    var v = (hiE * 2 + loE) / 3; // ideal interp_5_6 of selector 2
                    var e = Math.Abs(v - i) + Math.Abs(hiE - loE) * 3 / 100; // 3% GPU-approx term (ideal mode)
    ...
    static void GetBlockColors4(Span<int> br,
        Span<int> bg,
        Span<int> bb,
        int lr,
        int lg,
    ...
    static void FindSels4NoErr(scoped ReadOnlySpan<ColorRgba> px,
        int lr,
        int lg,
        int lb,
        int hr,
    ...
    static uint FindSels4FastErr(scoped ReadOnlySpan<ColorRgba> px,
        int lr,
        int lg,
        int lb,
        int hr,
        ...
        scoped Span<byte> sels,
        uint curErr)
        => Vector256.IsHardwareAccelerated ? FindSels4FastErrV256(px, lr, lg, lb, hr, hg, hb, sels, curErr) :
            Vector128.IsHardwareAccelerated ? FindSels4FastErrV128(px, lr, lg, lb, hr, hg, hb, sels, curErr) : FindSels4FastErrScalar(px, lr, lg, lb, hr, hg, hb, sels, curErr);
        ...
        scoped Span<byte> sels)
    {
        ...
        for (var i = 0; i < 16; i++)
        {
            var d = px[i].R * ar + px[i].G * ag + px[i].B * ab;
        ...
        uint curErr)
    {
        ...
        for (var i = 0; i < 16; i += 4)
        {
            for (var j = 0; j < 4; j++)
            {
                var d = px[i + j].R * ar + px[i + j].G * ag + px[i + j].B * ab;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static void Load4(scoped ReadOnlySpan<uint> pru,
        int g,
        Vector128<uint> mask8,
        out Vector128<int> r,
        out Vector128<int> gch,
        out Vector128<int> b)
    {
        var u = Vector128.Create(pru.Slice(g));
        r = (u & mask8).AsInt32();
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static Vector128<int> SelOf(Vector128<int> d,
        Vector128<int> t0,
        Vector128<int> t1,
        Vector128<int> t2,
        Vector128<int> three)
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static void Load8(scoped ReadOnlySpan<uint> pru,
        int g,
        Vector256<uint> mask8,
        out Vector256<int> r,
        out Vector256<int> gch,
        out Vector256<int> b)
    {
        var u = Vector256.Create(pru.Slice(g));
        r = (u & mask8).AsInt32();
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static Vector256<int> SelOf(Vector256<int> d,
        Vector256<int> t0,
        Vector256<int> t1,
        Vector256<int> t2,
        Vector256<int> three)
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static void PackSels(scoped ReadOnlySpan<int> si, scoped Span<byte> sels)
    {
        var lo = Vector128.Narrow(Vector128.Create(si), Vector128.Create(si.Slice(4)));
        var hi = Vector128.Narrow(Vector128.Create(si.Slice(8)), Vector128.Create(si.Slice(12)));
        Vector128.Narrow(lo.AsUInt16(), hi.AsUInt16()).CopyTo(sels);
    }
        ...
        scoped Span<byte> sels)
    {
        ...
        GetBlockColors4(br, bg, bb, lr, lg, lb, hr, hg, hb);
        int ar = br[3] - br[0], ag = bg[3] - bg[0], ab = bb[3] - bb[0];
        var t0 = (br[0] + br[1]) * ar + (bg[0] + bg[1]) * ag + (bb[0] + bb[1]) * ab;
        var t1 = (br[1] + br[2]) * ar + (bg[1] + bg[2]) * ag + (bb[1] + bb[2]) * ab;
        var t2 = (br[2] + br[3]) * ar + (bg[2] + bg[3]) * ag + (bb[2] + bb[3]) * ab;
        ar *= 2;
        ag *= 2;
        ab *= 2;
        ...
        var mask8 = Vector128.Create(0xFFu);
        var pru = MemoryMarshal.Cast<ColorRgba, uint>(px);

        Span<int> si = stackalloc int[16];
        for (var g = 0; g < 16; g += 4)
        {
            Load4(pru, g, mask8, out var R, out var G, out var B);
            SelOf(R * vAr + G * vAg + B * vAb, vT0, vT1, vT2, three).CopyTo(si.Slice(g));
        }
        ...
        scoped Span<byte> sels)
    {
        ...
        var mask8 = Vector256.Create(0xFFu);
        var pru = MemoryMarshal.Cast<ColorRgba, uint>(px);
        Span<int> si = stackalloc int[16];
        for (var g = 0; g < 16; g += 8)
        {
            Load8(pru, g, mask8, out var R, out var G, out var B);
            SelOf(R * vAr + G * vAg + B * vAb, vT0, vT1, vT2, three).CopyTo(si.Slice(g));
        }
        ...
        uint curErr)
    {
        ...
        Vector128<int> vT0 = Vector128.Create(t0), vT1 = Vector128.Create(t1), vT2 = Vector128.Create(t2);
        var three = Vector128.Create(3);
        var palR = Vector128.Create(br[0], br[1], br[2], br[3]);
        var palG = Vector128.Create(bg[0], bg[1], bg[2], bg[3]);
        var palB = Vector128.Create(bb[0], bb[1], bb[2], bb[3]);
        var mask8 = Vector128.Create(0xFFu);
        var pru = MemoryMarshal.Cast<ColorRgba, uint>(px);
        ...
        {
            Load4(pru, g, mask8, out var R, out var G, out var B);
            var sel = SelOf(R * vAr + G * vAg + B * vAb, vT0, vT1, vT2, three);
            sel.CopyTo(si.Slice(g));

            // palette[sel] per lane: sel in {0,1,2,3} indexes the 4-entry palette vector directly
            var dr = R - Vector128.Shuffle(palR, sel);
            var dg = G - Vector128.Shuffle(palG, sel);
            var db = B - Vector128.Shuffle(palB, sel);
        ...
        uint curErr)
    {
        ...
        var palR = Vector256.Create(br[0], br[1], br[2], br[3], 0, 0, 0, 0);
        var palG = Vector256.Create(bg[0], bg[1], bg[2], bg[3], 0, 0, 0, 0);
        var palB = Vector256.Create(bb[0], bb[1], bb[2], bb[3], 0, 0, 0, 0);
        var mask8 = Vector256.Create(0xFFu);
        var pru = MemoryMarshal.Cast<ColorRgba, uint>(px);
        ...
        for (var g = 0; g < 16; g += 8)
        {
            ...
            var sel = SelOf(R * vAr + G * vAg + B * vAb, vT0, vT1, vT2, three);
            sel.CopyTo(si.Slice(g));
            var dr = R - Vector256.Shuffle(palR, sel);
            var dg = G - Vector256.Shuffle(palG, sel);
            var db = B - Vector256.Shuffle(palB, sel);
    ...
    static void AccumCovariance(scoped ReadOnlySpan<ColorRgba> px,
        int avgR,
        int avgG,
        int avgB,
        out int c0,
    ...
    internal static void AccumCovarianceScalar(scoped ReadOnlySpan<ColorRgba> px,
        int avgR,
        int avgG,
        int avgB,
        out int c0,
    ...
    internal static void AccumCovarianceV128(scoped ReadOnlySpan<ColorRgba> px,
        int avgR,
        int avgG,
        int avgB,
        out int c0,
    ...
    static int HMin128(Vector128<int> m)
    {
    ...
    static int HMax128(Vector128<int> m)
    {
    ...
    static void ProjectMinMax(scoped ReadOnlySpan<ColorRgba> px, int sr, int sg, int sb, out int lowDot, out int highDot)
    {
    ...
    {
        lowDot = int.MaxValue;
        highDot = int.MinValue;
        for (var i = 0; i < 16; i++)
        {
            var dot = (px[i].R * sr + px[i].G * sg + px[i].B * sb & ~0xF) + i;
    ...
    internal static void ProjectMinMaxV128(scoped ReadOnlySpan<ColorRgba> px, int sr, int sg, int sb, out int lowDot, out int highDot)
    {
        ...
        {
            Load4(pru, g, mask8, out var R, out var G, out var B);
            var dot = (R * vSr + G * vSg + B * vSb & vNotF) + (iota + Vector128.Create(g));
            vLow = Vector128.Min(vLow, dot);
            vHigh = Vector128.Max(vHigh, dot);
        }

        lowDot = HMin128(vLow);
        highDot = HMax128(vHigh);
    }
    ...
    static bool ComputeLeastSquares(scoped ReadOnlySpan<ColorRgba> px,
    ...
    internal static bool ComputeLeastSquaresScalar(scoped ReadOnlySpan<ColorRgba> px,
        scoped ReadOnlySpan<byte> sels,
        int totalR,
        int totalG,
        int totalB,
    ...
    internal static bool ComputeLeastSquaresV128(scoped ReadOnlySpan<ColorRgba> px,
        scoped ReadOnlySpan<byte> sels,
        int totalR,
        int totalG,
        int totalB,
    ...
    {
        var mask8 = Vector128.Create(0xFFu);
        var wtab = Vector128.Create((int)WeightVals4[0], (int)WeightVals4[1], (int)WeightVals4[2], (int)WeightVals4[3]);
        Vector128<int> accR = Vector128<int>.Zero, accG = Vector128<int>.Zero, accB = Vector128<int>.Zero, accW = Vector128<int>.Zero;
        var pru = MemoryMarshal.Cast<ColorRgba, uint>(px);
        for (var g = 0; g < 16; g += 4)
        {
            Load4(pru, g, mask8, out var R, out var G, out var B);
            var sel = Vector128.Create(sels[g], sels[g + 1], sels[g + 2], sels[g + 3]);
            accR += sel * R;
            accG += sel * G;
            accB += sel * B;
        ...
        scoped Span<float> xh)
    {
        ...
        xl[0] = MulAdd(iz00, uq00R, iz01 * q10R);
        xh[0] = MulAdd(iz10, uq00R, iz11 * q10R);
        xl[1] = MulAdd(iz00, uq00G, iz01 * q10G);
        xh[1] = MulAdd(iz10, uq00G, iz11 * q10G);
        xl[2] = MulAdd(iz00, uq00B, iz01 * q10B);
        xh[2] = MulAdd(iz10, uq00B, iz11 * q10B);
        return true;
    }

    // float endpoints -> 565 with clamp + midpoint rounding. First triple from a, second from b.
    static void PreciseRound565(scoped ReadOnlySpan<float> a,
        scoped ReadOnlySpan<float> b,
        out int ar,
        out int ag,
        out int ab,
        out int br,
        ...
        out int bb)
    {
        ar = Clamp5((int)(a[0] * 31.0f));
        ag = Clamp6((int)(a[1] * 63.0f));
        ab = Clamp5((int)(a[2] * 31.0f));
        br = Clamp5((int)(b[0] * 31.0f));
        bg = Clamp6((int)(b[1] * 63.0f));
        bb = Clamp5((int)(b[2] * 31.0f));
        ar = ar + (a[0] > Midpoint5[ar] ? 1 : 0) & 31;
        ag = ag + (a[1] > Midpoint6[ag] ? 1 : 0) & 63;
        ab = ab + (a[2] > Midpoint5[ab] ? 1 : 0) & 31;
        br = br + (b[0] > Midpoint5[br] ? 1 : 0) & 31;
        bg = bg + (b[1] > Midpoint6[bg] ? 1 : 0) & 63;
        bb = bb + (b[2] > Midpoint5[bb] ? 1 : 0) & 31;
    }

    // grayscale endpoints (independent of the projection axis / power-iteration count).
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static void GrayscaleEndpoints(scoped ReadOnlySpan<ColorRgba> px,
        int minR,
        int maxR,
        out int lr,
        out int lg,
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static float MulAdd(float a, float b, float c)
    ...
    static void AxisFromCov(int icov0,
        int icov1,
        int icov2,
        int icov3,
        int icov4,
        ...
        out int saxisB)
    {
        ...
        {
            var r = MulAdd(xb, c2, MulAdd(xg, c1, xr * c0));
            var g = MulAdd(xb, c4, MulAdd(xg, c3, xr * c1));
            var b = MulAdd(xb, c5, MulAdd(xg, c4, xr * c2));
            xr = r;
            xg = g;
            xb = b;
        }

        var k = MathF.Max(MathF.Max(MathF.Abs(xr), MathF.Abs(xg)), MathF.Abs(xb));
        if (k >= 2)
        {
            var m = 2048.0f / k;
            saxisR = (int)(xr * m);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static void EndpointsFromAxis(scoped ReadOnlySpan<ColorRgba> px,
        int saxisR,
        int saxisG,
        int saxisB,
        out int lr,
        ...
        out int hb)
    {
        ProjectMinMax(px, saxisR, saxisG, saxisB, out var lowDot, out var highDot);
    ...
    static void PickInitialPca(scoped ReadOnlySpan<ColorRgba> px,
        bool grayscale,
        int minR,
        int minG,
        int minB,
        ...
        out int hb)
    {
        if (grayscale)
        {
            GrayscaleEndpoints(px, minR, maxR, out lr, out lg, out lb, out hr, out hg, out hb);
            return;
        }

        AccumCovariance(px, avgR, avgG, avgB, out var c0, out var c1, out var c2, out var c3, out var c4, out var c5);
        AxisFromCov(c0, c1, c2, c3, c4, c5, minR, minG, minB, maxR, maxG, maxB, powerIters, out var sr, out var sg, out var sb);
        EndpointsFromAxis(px, sr, sg, sb, out lr, out lg, out lb, out hr, out hg, out hb);
    }

    static void Encode4(scoped Span<byte> output,
        int lr,
        int lg,
        int lb,
        int hr,
        ...
        scoped ReadOnlySpan<byte> sels)
    {
        ...
        else
        {
            ...
            WriteEndpoints(output, lc16, hc16);
            ReadOnlySpan<byte> trans = stackalloc byte[]
            {
                0, 2, 3, 1
            };

            uint packed = 0;
            for (var i = 0; i < 16; i++) packed |= (uint)trans[sels[i]] << i * 2;
    ...
    static void WriteEndpoints(scoped Span<byte> output, uint color0, uint color1)
    {
        output[0] = (byte)color0;
        output[1] = (byte)(color0 >> 8);
        output[2] = (byte)color1;
        output[3] = (byte)(color1 >> 8);
    }

    static void EncodeSolid(scoped Span<byte> output, int fr, int fg, int fb)
    {
        uint mask = 0xAA;
        var max16 = Match5Hi[fr] << 11 | Match6Hi[fg] << 5 | Match5Hi[fb];
        var min16 = Match5Lo[fr] << 11 | Match6Lo[fg] << 5 | Match5Lo[fb];
        if (min16 == max16)
        {
    ...
    internal static void EncodeBlock(scoped ReadOnlySpan<ColorRgba> px, scoped Span<byte> output, Bc1Quality quality)
    {
        ...
        int j;
        for (j = 15; j >= 1; --j)
            if (px[j].R != fr || px[j].G != fg || px[j].B != fb)
                break;

        if (j == 0)
        {
            EncodeSolid(output, fr, fg, fb);
            return;
        }

        int totalR = fr, totalG = fg, totalB = fb, maxR = fr, maxG = fg, maxB = fb, minR = fr, minG = fg, minB = fb;
        var grayscale = fr == fg && fr == fb;
        for (var i = 1; i < 16; i++)
        {
            int r = px[i].R, g = px[i].G, b = px[i].B;
            grayscale &= r == g && r == b;
        ...
        else
        {
            ...
            var powerIters = quality == Bc1Quality.HighQuality ? 6 : 4;
            PickInitialPca(px,
                grayscale,
                minR,
                minG,
    ...
    static uint RunDescent(scoped ReadOnlySpan<ColorRgba> px,
        int totalR,
        int totalG,
        int totalB,
        int avgR,
        ...
        scoped Span<byte> sels)
    {
        ...
        var err = FindSels4FastErr(px, lr, lg, lb, hr, hg, hb, sels, uint.MaxValue);
        for (var pass = 0; pass < 2; pass++)
        {
            ...
            var trialErr = FindSels4FastErr(px, tlr, tlg, tlb, thr, thg, thb, trialSels, err);
            if (trialErr < err)
            {
                lr = tlr;
                lg = tlg;
